...
flowchart TB
  ...
  T2 --> T3["C6.6 BST → C6.7 平衡与AVL → C6.8 红黑树（待整理）"]
  T3 --> T4["C6.9 多路搜索树 → C6.10 B+树（待整理）"]
```
...
| C6.6 二叉搜索树（待整理） | part2 p.1–69 | 6.1 | BST 定义、find/insert/erase、前驱后继、第 k 个元素 |
| C6.7 平衡与 AVL 树（待整理） | part2 p.70–207 | 4.9 + AVL | 平衡的定义、红黑/权重平衡、AVL 的四种旋转、插入与删除 |
| C6.8 红黑树（待整理） | part2 p.208–295 | — | 性质、自底向上插入、自顶向下插入与删除 |
| C6.9 多路搜索树（待整理） | part3 p.1–51 | 6.4 | 3 路树、插入、中序遍历、多路树的规模与最小高度 |
| C6.10 B+ 树（待整理） | part3 p.52–175 | — | 硬盘的块寻址、M 路搜索树、B+ 树的查找/插入/删除 |

**阅读顺序**：C6.1 → C6.2 → C6.3 → C6.4 → C6.5，之后按表格顺序。**C6.5 按约定合并了课件的三个小节**（完美 / 完全 / N 叉）。tree_part1 的作业分别在 p.73（见 C6.3）和 p.97（见 C6.4）；**C6.1、C6.2、C6.5 三篇课件无作业页**。

> [!warning] 树这一章的两个术语陷阱（**考试重灾区**）
...
> - 第 6 章 树 Tree：课件共 620 页，拆为 10 篇。**tree_part1（150 页）已整理完成**，对应 C6.1–C6.5；**tree_part2（295 页）与 tree_part3（175 页）待整理**，对应 C6.6–C6.10。三个 pptx 均已转出同名 PDF 存于 pdfs/。
...
*课件来源：`pdfs/第1章：绪论.pdf`、`pdfs/第2章：线性表.pdf`、`pdfs/3.02.Stacks.pdf`、`pdfs/3.03.Queues.pptx`、`pdfs/3.04.Deques.pptx`、`pdfs/tree_part1.pptx`（均已转出同名 PDF）*
