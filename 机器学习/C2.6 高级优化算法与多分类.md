...
source: 课件 Lecture3 log reg1.pptx p22–31（该课件 p1–21 与 Lecture2-2.pdf 内容相同，已写在 [[C2.5 逻辑回归]]）；含该课件内嵌的 InkML 板书墨迹 8 页
---
...
> [!info] 怎么读这份讲义
> 第 1–2 节讲优化器，重点是**"你要交出什么，库负责什么"**这条接口；第 3–5 节讲多分类，重点是**为什么能拆、拆完怎么合回去**。
> 标有 **（拓展）** 或 **【拓展】** 的内容，是**课堂上没有展开讲、由课件和教材延伸补充**的部分。复习时可先掌握未标注的主干内容，行有余力再看拓展部分。

> [!note] 授课进度说明与板书来源
> 本篇对应的课件**暂无本校课堂语音转写稿**，因此「拓展」标注只标在明确属于教材/推导延伸的地方，未做全面区分。拿到第三节课转写稿后会按同一规则重新标注。
>
> **板书**：`log reg1.pptx` 是 **.pptx 原件**，内部保存了课堂板书墨迹（InkML 格式），本篇对应 8 页。**这些墨迹在导出成 PDF 时会被整体丢弃**，所以只看 PDF 是看不到的。本篇已把它们按原色（蓝/红/品红/青）重新渲染回幻灯片并逐节嵌入，callout 标题统一写作「**课堂板书**」。>
> **墨迹的来历**：这些墨迹的时间戳是 **2011 年 10 月**，说明它们是**该课件原讲者授课时留下的**，不是本学期课堂的录屏。所以它们的地位是「**课件自带的权威讲解**」——比课件印刷内容多，但**不能用来判断本学期课堂讲没讲到**，「拓展」标注仍以本校课堂语音转写稿为准。
...
> [!important] 课件原文：Optimization algorithm
...
课件把优化问题剥到了只剩骨头：**优化器不需要知道你在做逻辑回归还是线性回归，不需要知道什么是 sigmoid，不需要看见你的数据。** 它只需要你提供一个函数——给它一个 $\theta$，你返回两样东西：
...
> [!important] 课堂板书：这三行就是要你亲手交出去的东西
> ![[C2.6-板书-优化器接口.png]]
>
> 板书把 $J(\theta)$ 和 $\frac{\partial}{\partial\theta_j}J(\theta)$ **各自用红蓝双框框了两遍**，并各画一个粗箭头指向它们；同时在 $(j=0,1,\ldots,n)$ 的 $0$、$1$、$n$ 下面分别点了标记。
> 然后从 $\frac{\partial}{\partial\theta_j}J(\theta)$ 那个框引一条长曲线，一直指到下面梯度下降更新式里的 $\alpha\frac{\partial}{\partial\theta_j}J(\theta)$，那里同样被红蓝双框框住。
>
> **这条连线就是本节的中心思想**：你在上面算出来的那个梯度，就是下面更新式里要用的那一项；**优化器需要的全部输入，只有被框起来的这两样。**

### 2.2 四种优化算法与取舍

> [!important] 课件原文：Optimization algorithms / Advantages / Disadvantages
...
课件只写了这几行，课堂上会补的是**它们凭什么更快**，以及**"更复杂"复杂在哪**。
...
>
> **"更复杂"复杂在哪**：这几个算法都有几百行的实现细节（线搜索的 Wolfe 条件、数值稳定性、矩阵更新公式）。**结论是：不要自己实现，调库。** 课件后面给的就是调库的例子。
...
> 但接口完全一样：你在 PyTorch 里写 `loss.backward()` 交出梯度，`optimizer.step()` 由库决定怎么走——**正是课件这三行接口**。PyTorch 里也确实有 `torch.optim.LBFGS`，用于小规模全批量问题。

> [!important] 课堂板书：哪些是"可选项"，哪些是"必选项"
> ![[C2.6-板书-四种优化算法.png]]
>
> 板书用一个大方括号把 **Conjugate gradient / BFGS / L-BFGS 三项括在一起**，与 Gradient descent 分开（后者单独一个箭头）。
> 右侧把 "No need to manually pick $\alpha$" 与 "Often faster than gradient descent" 也用方括号括成一组，"More complex" 单独画箭头。
>
> **读法**：**梯度下降是一类，后三个是另一类**；后三个共享同一组优缺点（不用调 $\alpha$、更快、但复杂）。上面那两个被框住的 $J(\theta)$ 与梯度仍然被重复框出——**不论你换成哪一个算法，要交的东西一模一样。**

### 2.3 课件的 Octave/MATLAB 例子

课件用一个**玩具例子**演示怎么调库。这个例子故意选得极简，好让你把注意力放在"接口长什么样"上。

> [!example] 课件例子：$\theta$ 只有两个分量
...
> [!important] 课堂板书：一个课件没写、但会让你程序跑不起来的限制
> ![[C2.6-板书-fminunc例子与维度要求.png]]
>
> 板书在这一页补了三件事：
>
> **1. 先把答案写出来**。在 $J(\theta)=(\theta_1-5)^2+(\theta_2-5)^2$ 旁边直接写上
> $$\min_\theta J(\theta),\qquad \theta_1=5,\ \theta_2=5$$
> ——**先知道标准答案，再去看代码跑出来对不对**。
>
> **2. 把数学和代码逐行连起来**。用青色线把左边的 $\frac{\partial}{\partial\theta_1}J=2(\theta_1-5)$ 连到右边的 `gradient(1) = 2*(theta(1)-5);`，$\theta_2$ 那行同理；`jVal` 连到 $J(\theta)$ 的两个平方项。
>
> **3. 最实用的一条——`fminunc` 的维度要求**：在 `initialTheta` 下面写着
> $$\theta\in\mathbb{R}^d,\qquad d\ge 2$$
>
> > [!warning] 这是课件完全没写、但你第一次写代码就会撞上的坑
> > **`fminunc` 要求参数至少是 2 维向量，不接受标量。** 如果你的问题只有一个参数（比如只拟合 $\theta_0$），直接传一个数进去会报错，必须补成 $2\times1$ 的向量。
> > 这类"只有真正跑过代码才知道"的细节，正是板书最值钱的地方。

### 2.4 把它用到真正的逻辑回归上

课件最后一页把玩具例子换成一般情形：$\theta$ 是 $n+1$ 维。
...
> $\theta$ 的数学下标从 **0** 开始（$\theta_0,\theta_1,\ldots,\theta_n$，共 $n+1$ 个），而 Octave/MATLAB 的数组下标从 **1** 开始。
> 所以 `gradient(1)` 装的是 $\frac{\partial J}{\partial\theta_0}$，`gradient(n+1)` 装的是 $\frac{\partial J}{\partial\theta_n}$。**课件把这个错位明明白白写出来了，就是因为学生总在这里出错。**
>
...
> [!important] 课堂板书：数学下标与代码下标的对照表
> ![[C2.6-板书-theta下标错位.png]]
>
> 板书直接在参数向量旁边把对照关系一条条写出来：
...
> 并把代码里的 `gradient(1)`、`gradient(2)` 的编号**圈出来**，同时把右边 "code to compute $\frac{\partial}{\partial\theta_0}J(\theta)$"、"$\frac{\partial}{\partial\theta_1}J(\theta)$" 各自用红框框住、连到对应行。
>
> **课件只是把这个错位摆在那里，板书是把它一条一条点名写清楚的。** 正文 2.4 那条易错点提醒，板书用的是同一种做法：**与其记规则，不如把对照表抄一遍。**

## 3. 第二个缺口：多于两类怎么办

### 3.1 现实问题几乎都是多分类

> [!important] 课件原文：Multiclass classification
...
> [!note] 图怎么读
> 两张图坐标系相同（横轴 $x_1$、纵轴 $x_2$，都是特征空间，**纵轴不是标签**——这和 [[C2.5 逻辑回归]] 开头那张"肿瘤大小 vs 是否恶性"的图不一样，别混了）。
> 左图两簇用圆圈和叉区分，一条线就能切开；右图三簇用三角形、方块、叉区分，**一条直线无论怎么摆都切不出三块**。
...
> [!important] 课堂板书（两页）：类别编号与"一条线切不出三块"
> ![[C2.6-板书-多分类标签编号.png]]
>
> 第一页板书给三个例子**逐个编号**：
> - Email: Work / Friends / Family / Hobby → $y=1,2,3,4$（四个箭头分别指向四个词）
> - Medical: Not ill / Cold / Flu → $y=1,2,3$
> - Weather: Sunny / Cloudy / Rain / Snow → $y=1,2,3,4$
>
> 最下面还在 Weather 那行下面另写了一行 **$0,1,2,3$**。
> **这是在说：从 1 开始编还是从 0 开始编，都行，纯粹是约定。** 正文 3.1 表格里写的"$y\in\{1,2,\ldots,K\}$（或 $\{0,1,\ldots,K-1\}$）"，板书在这里给了实例。
>
> ![[C2.6-板书-二分类与多分类.png]]
>
> 第二页：课件左图（二分类）上**手画了一条分界线**穿过两簇之间——**课件原图没有这条线**；右图（三分类）只用三个箭头分别点出三簇，**一条线都没画**。
>
> **这个"画/不画"的对比本身就是结论**：二分类一条线就够，三分类画不出来——所以才需要下一节的 One-vs-all。
...
> [!important] 课件原文：One-vs-all (one-vs-rest)
...
> [!important] 课堂板书：三条分界线与三个 $h^{(i)}_\theta$
> ![[C2.6-板书-One-vs-all拆解.png]]
>
> 课件右侧那三张小图**原本只有散点，三条分界线全是板书画的**（品红色）：
> - 第一张：一条斜线把三角形那一簇分到一边，旁边框出 $h_\theta^{(1)}(x)$，并补写 $P(y=1\mid x;\theta)$；
> - 第二张：一条斜线把方块那一簇分出来，标 $h_\theta^{(2)}(x)$；
> - 第三张：一条竖线把 ✕ 那一簇分出来，标 $h_\theta^{(3)}(x)$。
>
> 左边原图上，三角形那一簇被品红圈单独圈出（对应第一个分类器的正类），Class 1/2/3 三行各画一个箭头。
> 底部那行 $h_\theta^{(i)}(x)=P(y=i\mid x;\theta)$ 被整个框起来。
>
> **三条线分别画出来，比文字更能说明"训练三个彼此独立的二分类器"是什么意思**——注意这三条线**各管各的，彼此没有任何约束**，这正是正文 4.2 里"三个输出加起来不等于 1"的几何来源。

### 4.2 怎么合回去

> [!important] 课件原文：One-vs-all
...
> 每个 $h_\theta^{(i)}(x)$ 都是**独立训练**出来的，它只回答"是不是第 $i$ 类"这个二选一问题，所以它自己内部满足 $P(y=i)+P(y\ne i)=1$。但三个分类器彼此之间**没有任何约束**，加起来不必等于 1，可能大于 1（三个都觉得像），也可能小于 1（三个都觉得不像）。
>
...
> [!question] 课堂追问：$K$ 类要训练几个分类器？
> $K$ 个。所以训练开销是二分类的 $K$ 倍。
>
> 顺带一提还有另一种拆法叫 **One-vs-one**：给每一对类别训练一个分类器，共 $\binom{K}{2}=\frac{K(K-1)}{2}$ 个，预测时投票。类别多时分类器个数增长更快，但每个分类器只用两类的数据、训练更快。**课件本讲只讲 One-vs-all。**（拓展）

> [!note] 一个真实的副作用：类别不平衡（拓展）
> 假设有 10 类、每类样本数相同。训练 $h^{(1)}$ 时，正类占 10%，负类占 90%——**One-vs-all 天然制造了类别不平衡**。如果不加处理，分类器容易倾向于全判负类。实践中会用类别权重、重采样等手段缓解。这是课件没提、但用起来一定会遇到的问题。

---

> [!important] 课堂板书：$\max$ 下面的那个 $i$
> ![[C2.6-板书-One-vs-all预测.png]]
>
> 板书把 $\max_i h_\theta^{(i)}(x)$ 里的**下标 $i$ 单独划线**，并在 $h_\theta^{(i)}(x)$ 下面画线、加一个向上的箭头；上文的 "class $i$"、"$y=i$" 也都划了线。
>
> **强调的是：这个 $\max$ 是"对 $i$ 取"的**——你比较的不是某一个分类器输出的大小，而是**把 $K$ 个分类器的输出排成一列，挑出最大的那一个所对应的类别编号**。写答案时 $\arg\max_i$ 的那个下标别丢。
...
| 四种算法 | Gradient descent / Conjugate gradient / BFGS / L-BFGS |
