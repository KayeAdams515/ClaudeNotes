...
aliases:
  ...
  - 系统与互联
source: Chapter 1-Signals and Systems.pdf (p.58–65) + 第四节课转写稿 + transcript/第五、六、七节课-已合并.txt（第五节课开头）
---
...
### Example 1.8（连续时间）：RC 电路 RC Circuit
...
>
> 5. **写成系统的标准形式**：输入是 $v_s(t)$，输出是 $v_c(t)$。把所有与输入相关的项放到**右手边**、与输出相关的项放到**左手边**；再按惯例用 $x(t)$ 记输入、$y(t)$ 记输出：
> $$\frac{dy(t)}{dt}+\frac{1}{RC}y(t)=\frac{1}{RC}x(t)$$
> 这就是系统的表达——**用一个方程描述输入与输出之间的关系**。
>
...
> 这就是"闭环 = 控制的基础"的由来。
>
> 把反馈框图里的**子系统 2 看成整个系统的控制器**：它监测系统的状态（输出），再调整进入子系统 1 的输入。所以 feedback 在**自动控制 (auto control)** 里用得非常多。

> [!tip] 开环 = 神经网络里的“前向”
> 开环系统的信号只朝一个方向走：输出受输入影响，但输入不受输出影响——这就是神经网络里 **feedforward（FFN / MLP，前向网络）** 的“前向”。而 RNN 带环路，就是闭环结构（它的稳定性问题见 [[C1.6 系统的基本性质]] 1.6.4）。

> [!warning] 考试提示
> 由串联、并联组合出的复杂开环结构，**第 2 章会出题**（求整体冲激响应）；反馈环结构相对少一些。对应的题型见 [[C2.3 LTI系统的性质]] 的随堂测第 2 题。

### 反馈互联实例 Example of Feed-back interconnection（课堂快速带过，定性理解即可）
...
>
> 定性地看：两端电压 $v(t)$ 的变化会改变流过电阻的电流 $i_2$，$i_2$ 又被从总电流里**减掉**，决定流进电容的 $i_1$——输出通过电阻“绕回来”影响输入，这就是反馈。框图里求和点是**减号**，正是因为 $i_1=i-i_2$。
