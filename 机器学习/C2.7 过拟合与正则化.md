...
> [!note] 板书来源与课件问题
> `log reg2.pptx` 是 **.pptx 原件**，内部保存了课堂板书墨迹（InkML 格式），**23 页里有 15 页带板书**。**这些墨迹在导出成 PDF 时会被整体丢弃**，所以只看 PDF 完全看不到。本篇已把它们按原色重新渲染回幻灯片，随对应知识点嵌入。
>
> 其中**有两页在 PDF 里是完全空白的**（对应正文 3.4 的 $\lambda$ 过大、4.2 的 pinv/inv），实际上整页内容都是手写的——**那两页空白不是排版失误，是墨迹被丢掉了。**
>
> **墨迹的来历**：时间戳是 **2011 年 10 月**，说明是**该课件原讲者授课时留下的**，不是本学期课堂录屏。所以它们是「**课件自带的权威讲解**」——比印刷内容多，但**不能用来判断本学期课堂讲没讲到**，「拓展」标注仍以本校课堂转写稿为准。
>
> 本篇还会遇到**两处公式在印刷和板书里都没写出来**（4.1、5.1，按数学补全）和**一处印刷与板书都写错的符号**（5.3），到了地方会逐处说明。
...
这三种情况各有名字，而且**名字就写在图下面**——只不过是手写的：

![[C2.7-板书-欠拟合与过拟合术语.png]]

**课堂板书**在三个模型下面一字不差地写着：左 **"Underfit" "High bias"**，中 **"Just right"**，右 **"Overfit" "High variance"**；右图那条四次曲线旁边还画了箭头指向它末端**急转直下**的那一段（**外推风险**，[[C2.4 多项式回归与正规方程]] 提过），下方正文里的 "$J(\theta)\approx0$" 和 "fail to generalize to new examples" 也都划了线。

> [!important] 三个术语，考试必须分清（术语是板书原话，图中现象是课件给的）
> | 情况 | 训练集表现 | 新样本表现 | 术语 |
> |---|---|---|---|
...
和上一页一样，**课件原图只有散点，三条边界全是手画的**：

![[C2.7-板书-逻辑回归过拟合.png]]

**课堂板书**在左图画了一条直线并写上 **"Underfit"**，中图画了一条光滑的 U 形闭合曲线，右图画的那条曲线最能说明问题——它**特意绕出两个细长的小环，去圈住混在圆圈堆里的那两个 ✕**，旁边写着 **"Overfit"**。

**"为了迁就个别样本而扭曲整体形状"——这就是过拟合的几何本质**，比任何文字定义都直观。
...
**课堂板书**用一个**贯穿 $x_1$ 到 $x_{100}$ 的大括号**把整列特征括成一个整体；右上角那张图的曲线也是手画的——**一条剧烈起伏、末端还掉头向下的曲线**穿过五个点。两件事合起来是一句话：**特征一多，拟合曲线就会变成这个样子。**
...
**课堂板书**在其中三行前面各画了一个箭头：人工挑特征、模型选择算法、以及"保留全部特征但压小 $\theta_j$"（$\theta_j$ 还划了线）。**唯独最后那句 "Works well when we have a lot of features…" 没有标箭头。**

**被标出来的是三个"具体动作"，没标的是一句"适用条件"。** 复习时先记住三个动作；而那句适用条件说清楚了正则化最擅长的场景——**特征很多、每个都有一点用**，这正是删特征最尴尬的情形。
...
课件印出来的只有这个**原始的**代价函数；真正的那一步是当场写上去的——

$$\min_\theta\ \frac{1}{2m}\sum_{i=1}^m(h_\theta(x^{(i)})-y^{(i)})^2\ +\ \underbrace{1000\,\theta_3^2+1000\,\theta_4^2}_{\text{板书手写补上的惩罚项}}$$
...
板书把这条推理链和它的效果，整个儿画在了同一页上：

![[C2.7-板书-惩罚theta3与theta4.png]]

**课堂板书**在两个惩罚项下面分别标 $\theta_3\approx0$、$\theta_4\approx0$，同时在右图的模型式子上**把 $\theta_3x^3$ 和 $\theta_4x^4$ 两项直接打了叉**，在保留下来的 $\theta_0+\theta_1x+\theta_2x^2$ 下面划线；右图里还**另画了一条品红色的平滑曲线**，叠在原来那条剧烈起伏的蓝色四次曲线上——**惩罚之后得到的就是那条平滑曲线**。

**两条曲线叠在同一张图上做对比，是这一页最值钱的信息，而它整个儿不在 PDF 里。**
...
这里有一个极容易被抄漏的细节，而板书正是在这一页把它用一个动作点了出来：

![[C2.7-板书-正则化代价函数由来.png]]

**课堂板书**先框住 "Small values for parameters $\theta_0,\theta_1,\ldots,\theta_n$" 整句，右侧单独框出 $\theta_3,\theta_4$ 并标 $\approx 0$（和上一页呼应）；然后在页面下方**手写出整个代价函数**，写完**在求和下标 $j=1$ 那里画了一个叉**，并在下面用红笔列出 $\theta_1,\theta_2,\theta_3,\ldots,\theta_{100}$ ——**前面的 $\theta_0$ 被单独打叉排除在外。**
...
>
> 实践上把 $\theta_0$ 也罚进去，结果差别通常很小；但**公式必须按课件写成 $j=1$ 到 $n$**，这是考点。这个"$\theta_0$ 特殊对待"的约定会在 4.1、4.2、5.2 里反复出现，请记住。
>
...
**课堂板书**在 $\lambda$ 旁边画箭头写上它的名字 **"regularization parameter"**，并用两个大括号把式子分成左右两块（数据拟合项 / 正则化项），各画一个向上的箭头——**就是上面那场拔河**。图里那条**品红色的平滑曲线**同样是画上去的，叠在原来那条剧烈起伏的蓝色曲线上，再一次用"两条曲线对比"展示正则化的效果。

既然 $\lambda$ 是个旋钮，就该问问：**把它拧到底会怎样？**
...
课件的下一页专门讲这件事，但**它在 PDF 里是整页空白**——因为整页内容都是手写的：
...
左侧画了四个 ✕ 样本点，上方画了**一条水平直线**并标注 **"Underfit"**，用箭头指出这条线就是 $h_\theta(x)$。（本讲义这一段是在拿到墨迹之前独立推出来的，两边一字不差地对上了。）
...
$\theta_0$ 单列一行、$(j=\cancel{0},1,2,\ldots,n)$ 的 0 上打着红叉（这是课件印刷内容）——**又是 3.3 那条规矩：$\theta_0$ 不加正则项。**

> [!warning] 方括号里的 $+\frac{\lambda}{m}\theta_j$ 是本讲义补的
> 课件 PDF 上第二行方括号里**只印出了求和项**（$\alpha$ 后面能明显看到一段空白），而这一页的板书也**没有把它写出来**。本讲义按标准形式补全，理由很硬：**不补上这一项，最后那行"整理后"的结果就推不出来**——而那一行是课件自己印出来的。

![[C2.7-板书-正则化梯度下降.png]]

这一页的**课堂板书**只有三处：把 $\theta_0$ 那行末尾的 $x_0^{(i)}$ **圈出来**（提醒 $x_0\equiv1$，所以这一项其实就是残差之和）、在 $\theta_j$ 那行的求和上方划一道横线、在 $\alpha$ 下方画一个向上的箭头。

那么多出来的 $\frac{\lambda}{m}\theta_j$ 到底从哪来？求一次导就知道：
...
讲课时，这个"缩水系数"被反复强调了一遍——**强调到讲下一页时还在写它**：

![[C2.7-板书-权重衰减0.99.png]]

这一页课件印的标题已经是 "Normal equation"，但**课堂板书**写的还是上一页的内容：
$$1-\alpha\frac{\lambda}{m}<1,\qquad \text{取个具体数字：}0.99,\qquad \theta_j\times 0.99$$
右侧还写了 $-\frac{\lambda}{m}\theta_j$、$\theta_j^2$、$J(\theta)$、$\frac{\partial}{\partial\theta_0}J(\theta)$，以及 $\theta_0$ 与 $\theta_1,\theta_2,\ldots,\theta_n$ 的分列。

**"每轮乘 0.99"——这是权重衰减最直白的说法**，一个具体数字胜过一段解释。
...
2. **标出各个矩阵的尺寸**：矩阵下方用大括号写 $(n+1)\times(n+1)$，$X$ 旁边标 $m\times(n+1)$，$y$ 旁边标 $\mathbb{R}^m$。
3. **点出这个公式是怎么来的**：上方写着 $\frac{\partial}{\partial\theta_j}J(\theta)\overset{\text{set}}{=}0$ ——**正规方程的来路是"令偏导为零"**（[[C2.4 多项式回归与正规方程]] 推过）。加了正则项之后还是这么做，只是多出 $\lambda\theta_j$ 一项，于是解里多出那个对角阵。

而 $\lambda>0$ 这个条件，顺手解决了一个老问题：
...
课件用一整页讲这件事，而这一页**在 PDF 里同样是整页空白**：

![[C2.7-板书-pinv与inv.png]]

**课堂板书**只有四个词，但正好把上面这段的对立面点了出来：
$$\textbf{non-invertible / singular}\qquad\qquad \textbf{invertible}$$
$$\texttt{pinv}\qquad\qquad\qquad\qquad\ \ \texttt{inv}$$
...
> [!warning] 这一页印出来的仍然是"没有正则化"的版本
> 它就是 [[C2.5 逻辑回归]] 里那个原版对数损失，**正则项不在这一页上**。完整形式是：
...
> **这不是推测，有两处可以互相印证**：一是**下一页的板书**把 $+\frac{\lambda}{2m}\sum_{j=1}^n\theta_j^2$ 手写了出来（见 5.2），二是课件**最后一页的印刷内容**里这个完整式子是印全了的（见 5.3）。
...
> $$(j=\cancel{0},1,2,3,\ldots,n)$$
> }
> （与 4.1 同样：0 上的红叉是课件印刷内容；方括号内的 $+\frac{\lambda}{m}\theta_j$ 印刷和板书都没写出来，按数学补全。）

![[C2.7-板书-正则化逻辑回归梯度.png]]

**课堂板书**在这一页做了两件事。第一件是**把 5.1 缺的那一项补了出来**：
$$+\ \frac{\lambda}{2m}\sum_{j=1}^{n}\theta_j^2$$
并在旁边框出它作用的范围 $\boxed{\theta_1,\theta_2,\ldots,\theta_n}$ ——**再一次强调不含 $\theta_0$**（本讲第四次出现同一件事：3.3 打叉、4.1 单列一行、4.2 左上角那个 0、这里框出范围）。

第二件是把 $\theta_0$ 那一整行**涂掉了**。**涂掉不是说它不用更新**，而是：它的更新式和没有正则化时完全一样，**没有新东西可讲**，所以划掉、只看下面那行。
...
课件最后把 [[C2.6 高级优化算法与多分类]] 的接口和正则化拼在一起——这一页也是本讲**唯一一处真正的笔误**所在。
...
> ```
>
> 注意第一行的 $J(\theta)$：**这里印的是带 $\frac{\lambda}{2m}\sum\theta_j^2$ 的完整式子**，正好补上 5.1 那一页的缺口。

![[C2.7-板书-正则化高级优化.png]]

**课堂板书**把 `gradient(1)` 那一行的 $\frac1m\sum(h_\theta(x^{(i)})-y^{(i)})x_0^{(i)}$ 用括号框起来，并从右侧拉一条长线过来写上 $\frac{\lambda}{m}\theta_j$ 那一项——意思是**"从第二个梯度开始才多这一项，第一个（对应 $\theta_0$）没有"**；右下角另写 $h_\theta(x)=\frac{1}{1+e^{-\theta^Tx}}$，提醒这里的 $h$ 是 sigmoid 不是 $\theta^Tx$。

但那一项的符号，印刷和板书都写错了：

> [!warning] 印刷与板书都写成了减号，数学上必须是加号
> 课件把后面几个梯度印成 $\cdots-\frac{\lambda}{m}\theta_1$（**减号**），**板书也照着写了减号**。两边一致，说明这是**原课件本身的笔误、讲课时没有发现**，而不是导出过程弄丢了什么。
>
> **正确的是加号 $+\frac{\lambda}{m}\theta_1$**，三条独立证据：
...
课件的最后一页在 PDF 里几乎全白，**课堂板书**在上面重写了一遍 [[C2.6 高级优化算法与多分类]] 那张下标对照表：
