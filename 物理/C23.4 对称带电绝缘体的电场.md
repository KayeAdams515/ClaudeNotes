...
> [!abstract] 这一讲的任务
> 上一讲的导体好办，是因为它自带"内部 $\vec E=0$"这条强性质。**绝缘体没有**：电荷被钉死在原地，内部也有场。
> 这一讲要回答：**没有那条强性质，高斯定律还能用吗？** 能——但必须靠**对称性**，而且只有三种情形能解出来。我们会把这三种一网打尽（球、柱、面），并在球壳那里看到一件痛快的事：**第 21 章用两页积分证明的壳层定理，这里两行就出来了。**

> [!info] 怎么读这份讲义
> 第 1 节的"匹配"标准是选高斯面的唯一依据，不是审美问题。第 2–4 节是三种对称性，做完每一个都要做**边界连续性检验**。第 5 节的汇总表和"维度—衰减律"那条规律值得背下来。
> 标有 **（拓展）** 或 **【拓展】** 的内容，是**课堂上没有讲到、由课件和教材延伸补充**的部分。复习时可先掌握未标注的主干内容，行有余力再看拓展部分。

```mermaid
graph TD
    A["绝缘体：内部有场，电荷不能动"] --> B["没有'E=0'可用<br/>只能靠对称性"]
    B --> C["匹配标准：每块面上<br/>要么 E∥dA 且恒定，要么 E⊥dA"]
    C --> D["球对称：同心球面"]
    C --> E["柱对称：同轴圆柱"]
    C --> F["面对称：跨面药盒"]
    D --> D1["壳层定理两行搞定<br/>球体内 E ∝ r"]
    E --> E1["E ∝ 1/r"]
    F --> F1["E = σ/2ε₀，均匀场"]
    D1 --> G["维度 ↑ 一维，衰减慢一个 1/r"]
    E1 --> G
    F1 --> G
```
...
第 21 章我们花了整整两页，证明"**均匀带电球壳对壳外粒子的作用，就像全部电荷集中在球心**"（壳层定理）。过程你大概还记得：把球壳切成环带、换元 $x\,dx=rR\sin\theta\,d\theta$、算 $\int_{r-R}^{r+R}(\cdots)dx=4R$。

> [!question] 停一下，先自己想想
> 现在用高斯定律重做这道题。球壳是球对称的，所以场只能沿径向、且在同一个球面上大小相同。取一个半径 $r>R$ 的同心球面：
> $$E\cdot4\pi r^2=\frac{q}{\varepsilon_0}$$
> **就完了。** 两行。
>
> 同样的物理，为什么差这么多？

差别在于：第 21 章是**局部求和**——老老实实把球壳上每一小块的贡献算出来再加；高斯定律是**整体记账**——根本不问球壳上电荷怎么摆，只数有多少条电场线穿出了那个球面。

**但这不是白得的。** 能这样"只数总数"，前提是我们**事先就知道场在整个高斯面上处处等大、方向一致**——这个信息来自**对称性**，不是来自高斯定律。对称性不够的时候（有限长棒、方板），这一步就垮了，高斯定律虽然照样成立，却解不出任何东西。

**所以这一讲的真正内容是：什么样的对称性配什么样的高斯面。**
...
## 2. 球对称 Spherically symmetric charges 球对称
...
> 开场那个对比现在兑现了：第 21 章证明这两条壳层定理用掉**两页积分**（切环带、换元 $x\,dx=rR\sin\theta\,d\theta$、算 $\int_{r-R}^{r+R}(\cdots)dx=4R$），这里**每条只用两行**。
...
> [!question] 停一下，先自己想想
> 往球心走的路上，场是变大还是变小？
> 两股力量在打架：距离变近（$1/r^2$ 要变大），可被高斯面圈住的电荷也变少了（$q_{enc}$ 要变小）。**谁赢？**
...
## 3. 柱对称 Cylindrically symmetric charges 柱对称
...
> [!question] 课堂追问：高度 $h$ 去哪了？为什么这次是 $1/r$ 不是 $1/r^2$？
> **$h$ 消掉是必然的**：它同时出现在"侧面积 $2\pi rh$"和"包围电荷 $\lambda h$"里，一定约掉——**结果不依赖高斯面取多高**。这是柱对称的自洽性检验，如果你算出来的 $E$ 里还带着 $h$，一定错了。
>
> **衰减律为什么变了**：因为"场线摊开"的方式不同。点电荷的线摊在**球面**上（面积 $\propto r^2$），无限长线的线摊在**柱侧面**上（面积 $\propto r$）。通量守恒，所以 $E$ 反比于那个面积。
> **几何决定衰减律**——这与 [[C22.2 点电荷的电场]] 的"线密度"论证是同一件事。
...
## 4. 面对称 Planar symmetric charges 平面对称
...
1. **高斯面要"匹配"对称性**，标准只有一条：每块面上要么 $\vec E$ 恒定且平行于 $d\vec A$，要么 $\vec E\perp d\vec A$。对称性论证是评分点，不能省。
2. **三种可解对称性**：球（同心球面）、无限长柱（同轴圆柱）、无限大面（跨面药盒）。内部均匀带电时都是 $E\propto r$，且在边界与外部公式**连续**——这是必做的自检。
3. **维度决定衰减律**：点 $1/r^2$、线 $1/r$、面 $r^0$；源的维度每升一维，衰减慢一个 $1/r$。

> [!important] 必背
> $$E_{\text{球壳外}}=\frac{q}{4\pi\varepsilon_0r^2},\quad E_{\text{球壳内}}=0,\quad E_{\text{绝缘球内}}=\frac{qr}{4\pi\varepsilon_0R^3},\quad E_{\text{线}}=\frac{\lambda}{2\pi\varepsilon_0r},\quad E_{\text{板}}=\frac{\sigma}{2\varepsilon_0}$$

> [!abstract] 下一章预告
> 第 23 章给了我们一件很趁手的工具：给定对称的电荷分布，三行算出 $\vec E$。但 $\vec E$ 是**矢量**——每一点三个分量，叠加要分解、要检查象限，麻烦始终在。
> 第 24 章要问一个大胆的问题：**能不能把整个静电场的信息装进一个只有大小、没有方向的数里？** 听起来像要丢掉一半信息，但下一章会证明：**什么也没丢**，而且整套矢量工序从此作废。
...
> [!success]- 参考答案
> $q_{enc}\propto r^3$ 增长快于 $1/r^2$ 的衰减，净效果 $E\propto r^3/r^2=r$。检验：$r=0$ 给 0 ✓，$r=R$ 与外部公式相等 ✓。

> [!question] 4. 实心导体球与实心绝缘球，内外场有何异同？

> [!success]- 参考答案
> 外部完全相同（$q/4\pi\varepsilon_0r^2$）；内部：导体 $E=0$，绝缘体 $E\propto r$。所以题目必须先说明是导体还是绝缘体。

> [!question] 5. 无限长带电线的场为什么按 $1/r$ 衰减？高斯面的高度 $h$ 为什么会消掉？

> [!success]- 参考答案
> 场线摊在柱侧面上（面积 $\propto r$）而非球面上。$h$ 同时出现在侧面积 $2\pi rh$ 和包围电荷 $\lambda h$ 中，必然约掉——结果不依赖高斯面取多高。

> [!question] 6. 无限大绝缘板 $\sigma/2\varepsilon_0$ 中的因子 2 从哪来？

> [!success]- 参考答案
> 药盒的**两个**底面都出通量（场向两侧对称发出），通量被平分。导体表面只有 1 个底面出通量（内侧 $E=0$），故为 $\sigma/\varepsilon_0$。

> [!question] 7. "无限大平面"的实用判据是什么？

> [!success]- 参考答案
> 场点到板的距离远小于板到边缘的距离——站在场点看不到边界。极限的本质是无量纲比值小，不是绝对尺寸大。
...
> [!question] 9. 遇到一根有限长的带电棒，还能用高斯定律求场吗？

> [!success]- 参考答案
> 高斯定律仍然**成立**，但**解不出**：两端破坏柱对称，高斯面上 $E$ 不再处处相等、底面通量也不为零，$E$ 无法从积分号里提出来。只能回到 [[C22.4 线电荷的电场]] 的微元积分法。

---

上一节 ← [[C23.3 带电导体的电场]] ｜ 下一章 → [[C24.0 开篇回顾 第21章遗留习题]]
索引 → [[电磁学 MOC]]
