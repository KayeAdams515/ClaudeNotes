...
source: 课件 lecbig.pdf p17–24（**含 5 页课堂板书**）
---
...
>
> 板书在 $p(y=1\mid x;\theta)$ 下方引出箭头写 **price**（$x$ 里包含价格），右边写 **logistic regression**——这就是一个逻辑回归问题。然后写出算法：
>
> **Repeat forever {**
...
> [!important] 和 SGD 对比
> 更新式和 [[C8.1 大数据集与随机梯度下降]] 的 SGD **一字不差**（逻辑回归的梯度形式与线性回归相同，见 [[C2.5 逻辑回归]]）。区别只在于**数据从哪来**：
>
> | | SGD | 在线学习 |
> |---|---|---|
...
**为什么板书划掉了 $(x^{(i)},y^{(i)})$**：在线学习里**没有“第 $i$ 个样本”这个概念**。样本不存储、不编号、不重复使用——用户来了，学一下，就过去了。这就是“在线”的含义。

> [!tip] 什么时候适合在线学习
> **数据流足够大**时。一个大型网站每天有成千上万的用户，数据多到根本不需要重复使用旧样本——新数据永远比旧数据多。如果用户很少（一天几个），就不如把数据存下来，用普通方法训练。
...
> 价格定高了，每单利润多但接受概率低；定低了反之。对每个用户，**枚举几个候选价格，选期望利润最大的那个报出去**。课件说的 “optimize price” 指的就是这一步。
...
>
> 板书把几乎每一句都划了线：搜索词 “**Android phone 1080p camera**”、**100** phones in store、**Will return 10 results**（红线）；$x$ 的三类特征各自划线；在 “Learn $p(y=1\mid x;\theta)$” 旁写 **predicted CTR**；右侧写 $(x,y)$ 并加红箭头；最后一段的 special offers、news articles、product recommendation 都划了线。
...
**在线学习怎么发生**：每次展示 10 个结果，用户点了其中某些、没点其他的——**一次搜索就产生 10 个 $(x,y)$ 样本**。每个都拿来更新一次 $\theta$，然后扔掉。

**其他例子**（课件原文）：
- 选择给用户展示哪些**特惠 special offers**；
- **新闻文章**的个性化选择；
- **商品推荐**（→ 这正是 [[C7.1 推荐问题与基于内容的推荐]] 的场景，推荐系统天然适合在线学习）。
...
（为了好算，假设 $m=400$。）这个求和有 400 项。**把它拆成 4 份，交给 4 台机器同时算**：
...
>
> 理想情况下，4 台机器能把计算提速接近 **4 倍**（实际会略少，因为要把数据分发出去、把结果收回来，有网络通信开销）。
...
MapReduce 这个框架出自 Google 的 Jeffrey Dean 和 Sanjay Ghemawat（2004），开源实现是 Hadoop。（**拓展**）
...
> ![[C8.3-板书-求和形式的算法.png]]
>
> 板书在两个求和号下方各画了一个弯箭头，并分别写 **$\text{temp}^{(i)}$** 和 **$\text{temp}_j^{(i)}$**——**代价函数和梯度都可以拆成若干个 temp 的和**。

> [!warning] 课件笔误：$J_{train}$ 的符号
> 课件 p23 印的是
...
>
> 所以大规模场景下常见两种路线：**单机 SGD / 小批量**（每步便宜，步数多），或**多机 MapReduce + 批量法**（每步贵但可并行）。小批量梯度下降也可以把一个 batch 拆给多个核算，介于两者之间。
...
>
> 更进一步：很多线性代数库在做**向量化运算**时，**已经自动用了多核**。这时你只要把代码写成向量化形式（[[C8.2 小批量梯度下降与 SGD 的收敛]] 1.3），并行就自动发生了，连 MapReduce 都不用手写。（**拓展**）

---

## 3. 停一下，我们刚才干了什么

整个第 8 章是一张“数据太大怎么办”的工具箱：
...
> [!important] 三句话
> 1. **在线学习**：数据源源不断时，`Repeat forever { 获取当前用户的 (x,y)；用它更新 θ：θ_j := θ_j − α(h_θ(x) − y)x_j }`——更新式与 SGD 相同，但**没有固定训练集、样本不编号、用完即扔**（板书划掉了 $(x^{(i)},y^{(i)})$），并且**能适应用户偏好的变化**。课件例：快递网站学 $p(y=1\mid x;\theta)$（$x$ 含报价）以优化定价；商品搜索学**预测点击率 CTR** 以决定展示哪 10 款手机；还可用于特惠选择、新闻推荐、商品推荐。
