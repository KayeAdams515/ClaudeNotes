...
> [!warning] 📚 课堂进度提示
> **本节 §1 的“永真式 / 矛盾式 / 可能式”，老师已经在 Ch1.2 的课件里讲过了**（见 [[C1.2 命题逻辑的应用 Applications of Propositional Logic]] §4.5）。
...
> **§6 可满足性、§7 数独、§8 SAT 求解课堂上没有讲**，已标注为**（拓展）**。课件独有的推导方法已并入 §3–§5 正文，标 📚。
...
> [!warning] 📚 课件：真值表为什么不够用（slide 7）
> 课件讲完真值表法之后，专门用一张幻灯片算它的代价：**$n$ 个变元要写 $2^n$ 行**，
> $$n=20 \Longrightarrow 2^{20}=1{,}048{,}576\ \text{行}$$
> 结论是 **Not efficient**——所以才要引入下面一整套等价律。
> 记住 $2^{20}\approx10^6$ 这个量级：**每多一个变元，工作量翻一倍**。§8 里 SAT 问题难在哪，是同一个原因。
...
> [!note] 🎤 课堂：“幂等律”这个名字的来历
> 课堂原话：**“幂就是重复的意思，像 $2^2=2\times2$、$2^3=2\times2\times2$。”**
> 所以 $p\vee p\equiv p$、$p\wedge p\equiv p$ 说的就是“同样的东西重复多少遍都一样”。
> 实用价值：**把相同的变量凑到一起就能约掉多余的**——下面推 TABLE 7 第 ⑦ 条时正是卡在这一步。

> [!warning] 🎤 课堂：“$\wedge$ 像乘号”这个类比的边界
> 讲分配律时课堂上先用了“$\wedge$ 有点像乘号、$\vee$ 有点像加号”来帮助理解，随后**当场纠正**：
> **“你不能想象 and 就等价于乘法。”**
> 因为 $\vee$ 对 $\wedge$ **也**有分配律（$p\vee(q\wedge r)\equiv(p\vee q)\wedge(p\vee r)$），而算术里加法对乘法**没有**分配律。
> **正确的记法**：只看**括号外面**是哪个运算符；**括号内外运算符不同**时才用分配律。

> [!warning] 🔧 课件勘误（课堂当场更正）
> 讲 Identity Laws（$p\wedge\mathbf T\equiv p$、$p\vee\mathbf F\equiv p$）那一页，课件配的小真值表写错了，课堂上当场重画并致歉。
> **等式本身没问题，以上面 TABLE 6 为准。**
...
> [!important] ⭐⭐ 📚🎤 课件与课堂的核心方法：TABLE 7 **不用背，只背第一条**
> 课件在 TABLE 7 上用红字写着 **“You only need to memorize this.”**，指的就是
> $$\boxed{P \to Q \equiv \neg P \vee Q}$$
> 其余八条**课堂上当堂逐条推导**给大家看。课堂反复用的比喻：
> **“这个等式是把箭头式的东西变成 or / and 形式的一座桥梁。”“有了它，所有 operator 之间的关系就完全打通了。”**
> 也就是说：**它是进出‘箭头世界’的唯一那扇门。**

> [!note] 停一下：八条推导其实是同一个三步套路
> 1. **用 $P\to Q\equiv\neg P\vee Q$ 把所有箭头消成 $\vee$**；
> 2. **在纯 $\wedge\vee\neg$ 的世界里用分配律 / 德摩根律 / 幂等律整理**；
> 3. **再用同一条把结果装回箭头。**
>
> 所以 TABLE 7 **考场上现推即可**，每条不超过四行。课堂全程没有要求背 TABLE 7/8，**但要求会推**——所以考试更可能出“推导题”而不是“默写题”。

> [!example] 📚 课件逐条推导（slide 23–24）：① $P\to Q \equiv \neg Q\to\neg P$（逆否律）
> 令 $P = \neg S$，$Q = \neg R$：
> $$\begin{aligned}
> P\to Q &\equiv \neg P\vee Q && \text{消箭头}\\
> &\equiv \neg(\neg S)\vee\neg R && \text{代换}\\
> &\equiv S\vee\neg R && \text{双重否定律}\\
> &\equiv \neg R\vee S && \text{交换律}\\
> &\equiv R\to S && \text{消箭头（反向用）}\\
> &\equiv \neg Q\to\neg P && \text{代换回去}
> \end{aligned}$$
>
> > [!warning] 🔧 课件勘误 + ⭐ 答题红线：代换时不要用 $T$、$F$ 做字母
> > 课件原文用的是“令 $P=\neg S$、$Q=\neg T$”，课堂上讲完立刻自我更正：
> > **“但是我这里有个严重的错误——我用了 $T$。因为 $T$ 跟 $F$ 在离散数学里有特殊含义，如果我们在考试这么写，那可能就要扣分了。”**
> > **所以本笔记已把 $T$ 换成 $R$；你自己答题时也请用 $S,R$ 或 $A,B$，避开 $\mathbf T,\mathbf F$。**

> [!example] 📚 ② $P\vee Q \equiv \neg P\to Q$
> 令 $P = \neg S$：$\quad P\vee Q \equiv \neg S\vee Q \equiv S\to Q \equiv \neg P\to Q$（中间一步是消箭头反过来用）。

> [!example] 📚 ③ $\neg(P\to Q)\equiv P\wedge\neg Q$
> $$\neg(P\to Q)\equiv\neg(\neg P\vee Q)\equiv P\wedge\neg Q\quad\text{（消箭头 + 德摩根律 + 双重否定）}$$
...
> [!important] 📚 课件：TABLE 8 同样**只背第一条**
> $$\boxed{P \leftrightarrow Q \equiv (P\to Q)\wedge(Q\to P) \equiv (\neg P \vee Q)\wedge(\neg Q\vee P)}$$
> 其余三条课件当堂推导（slide 26–28），套路与 TABLE 7 完全一样：**先化到 $\wedge\vee\neg$，整理，再装回去。**

> [!example] 📚 ⑨ $P\leftrightarrow Q\equiv(P\wedge Q)\vee(\neg P\wedge\neg Q)$
> $$\begin{aligned}
> P\leftrightarrow Q &\equiv(\neg P\vee Q)\wedge(\neg Q\vee P) && \text{双条件展开}\\
...
> 令 $P=\neg S$，$Q=\neg R$：
> $$P\leftrightarrow Q\equiv(\neg P\vee Q)\wedge(\neg Q\vee P)\equiv(S\vee\neg R)\wedge(R\vee\neg S)\equiv S\leftrightarrow R\equiv\neg P\leftrightarrow\neg Q$$

> [!example] 📚 ⑪ $\neg(P\leftrightarrow Q)\equiv P\leftrightarrow\neg Q$
> $$\begin{aligned}
> \neg(P\leftrightarrow Q) &\equiv\neg\bigl((\neg P\vee Q)\wedge(\neg Q\vee P)\bigr) && \text{双条件展开}\\
> &\equiv\neg(\neg P\vee Q)\vee\neg(\neg Q\vee P) && \text{德摩根律}\\
> &\equiv(P\wedge\neg Q)\vee(Q\wedge\neg P) && \text{德摩根律}\\
> &\equiv\cdots\equiv P\leftrightarrow\neg Q && \text{（分配律 + 否定律 + 同一律，同 ⑨）}
> \end{aligned}$$
> **注意**：$(P\wedge\neg Q)\vee(\neg P\wedge Q)$ 正是 $P\oplus Q$ 的 DNF，所以这条也说明 $\neg(P\leftrightarrow Q)\equiv P\oplus Q$。
...
> [!important] 📚 课件把推广的推导写全了（slide 18–20）——**考试若要求“证明推广形式”，照抄这个过程**
...
> **第四步**：如此反复，最终得到上面两条推广式。
>
> 这个推导的结构是“**把 $n$ 的情形化归到 $n-1$，再反复化归**”——正是**数学归纳法**（第 5 章）的雏形。课件用的是“$\cdots$”的非正式写法，学了归纳法之后可以写成完全严格的形式。
...
> [!warning] ⭐⭐ 🎤 课堂：推导题的答题规范（本节课强调得最重的一件事）
> 课堂在进入练习前专门花了几分钟讲格式，**这是全课最明确的一次得分点提示**：
> - **“你在期中考试或期末考试的时候，一定有推导题，这个毋庸置疑——这个太好出试卷了。”**
> - **“推导的每一个步骤，你必须要写清楚你用的是什么定律。”**
> - **“如果你记不住那些名字（摩根肯定是人名，我都记不住），没问题——你可以把这个定律的公式写在旁边，这样我就知道你是什么意思了。”**
>
> **格式**：`每一步的表达式　|　右侧写律名 或 该律的等式`。两者任选其一，**但不能不写**。
>
> 两个容易忽略的细节：
> **①** 题目里的变量未必是 $p,q,r$——往年考题用过 $x,y,z$。“你用 $p,q$ 也可以，$x,y,z$ 也可以，**但你总得写个公式在那儿**。”
> **②** $\mathbf T$ 和 $\mathbf F$ **绝不能当变量用**（见上面 TABLE 7 ① 的勘误框）。

> [!example] 📚 课件例题（slide 8）：$(p\to q)\vee(p\to r)\equiv p\to(q\vee r)$ 的两种做法
> **做法 A（等价变形）**
> $$(p\to q)\vee(p\to r)\equiv(\neg p\vee q)\vee(\neg p\vee r)\equiv(\neg p\vee\neg p)\vee(q\vee r)\equiv\neg p\vee(q\vee r)\equiv p\to(q\vee r)$$
> **做法 B（真值表）**：列 8 行，比对两列完全相同。
>
> **要点**：考试若没指定方法，**优先用做法 A**——三行搞定，真值表要写 8 行 6 列。
> 课件另外还用真值表先验证过 $\neg p\vee q\equiv p\to q$（4 行）与 $p\vee(q\wedge r)\equiv(p\vee q)\wedge(p\vee r)$（8 行），再把它们收进 TABLE 6/7——**先验证、再入表**的顺序。
...
> 来源：课件 `Ch1.3`（2026-09-08，29 张）＋ 课堂录音转写稿第 4 节课。
...
> **题目**：用德摩根律写出下列命题的否定，并用英语表述。
> 7a) Jan is rich and happy.　7b) Carlos will bicycle or run tomorrow.
> 7c) Mei walks or takes the bus to class.　7d) Ibrahim is smart and hard working.
> 8a) Kwame will take a job in industry or go to graduate school.　8b) Yoshiko knows Java and calculus.
>
> > [!success]- 参考答案
...
> **题目**：用真值表证明下列各式是永真式；再按习题 11 的要求，不用真值表重证一遍。
> 9c) $\neg p \to (p \to q)$　　9e) $\neg(p \to q) \to p$
>
> > [!success]- 参考答案
> > **9c) $\neg p \to (p \to q)$**
...
>
> **题目**：
> 34. 求对偶：a) $p \vee \neg q$　b) $p \wedge (q \vee (r \wedge \mathbf{T}))$　c) $(p \wedge \neg q) \vee (q \wedge \mathbf{F})$
> 35. 求对偶：a) $p \wedge \neg q \wedge \neg r$　b) $(p \wedge q \wedge r) \vee s$　c) $(p \vee \mathbf{F}) \wedge (q \vee \mathbf{T})$
> 36. 何时有 $s^* = s$？　37. 证明 $(s^*)^* = s$。
> 38. 说明 Table 6 的等价律为什么成对出现（双重否定律除外）。
> 39.（∗∗）证明：若 $s \equiv t$，则 $s^* \equiv t^*$。
>
> > [!success]- 参考答案
...
>
> **题目**：
> 46 & 48. 构造 $p \mid q$ 与 $p \downarrow q$ 的真值表。
> 47. 证明 $p \mid q \equiv \neg(p \wedge q)$；49. 证明 $p \downarrow q \equiv \neg(p \vee q)$。
> 50. 证明 $\{\downarrow\}$ 功能完备（先造 $\neg$，再造 $\vee$）。　51. 只用 $\downarrow$ 表示 $p \to q$。
> 52. 证明 $\{\mid\}$ 功能完备。　53. 证明 $p \mid q \equiv q \mid p$。　54. 证明 $\mid$ 不满足结合律。
>
> > [!success]- 参考答案
> > **46 & 48. 真值表**
> >
...
> **题目**：仿照课本对 9×9 数独的命题逻辑编码，给出 4×4 数独（由四个 2×2 块组成，每格填 1–4）的编码——用变元 $p(i,j,n)$ 写出一个合取式，使它的满足赋值恰好对应该数独的解。
>
> > [!success]- 参考答案
