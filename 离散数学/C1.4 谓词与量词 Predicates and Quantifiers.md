...
> 课件独有的内容（自由/约束变元对照表、量词分配律的反例表、含常量 $A$ 的等价式、受限论域的三种写法）见文末「📚 课件补充」。
...
# 📚 课件补充 Lecture Slides（Dr. 欧士琪 Ch1.4 Part 1 & 2）

> [!info] 课件信息
> **Discrete Mathematics · Chapter 1: Logic and Proof · 1.4 Predicates and Quantifiers**
> 主讲：**Dr Shiqi (Shawn) Ou, 欧士琪**，South China University of Technology
> **Part 1**：2026 年 9 月 9 日，29 张　|　**Part 2**：2026 年 9 月 10 日，29 张
>
> **课件 Agenda（六条主线）**
> 1. Predicates（谓词）　2. Quantifiers（量词）　3. Quantifiers with Restricted Domains（受限论域）
> 4. Precedence of Quantifiers（量词优先级）　5. Logical Equivalences Involving Quantifiers（含量词的等价）　6. Translation（翻译）

---

## L1. 课件的开场：命题逻辑的**两个**局限（Part1 slide 3、9–10）

教材只含糊地说"命题逻辑表达力不够"。课件把它拆成了**两个具体的、可以感受到的**局限。

> [!important] Limitation 1：**相似的命题只能各起一个名字**
> $$p:\ \text{John is a SCUT student}\qquad q:\ \text{Peter is a SCUT student}\qquad r:\ \text{Mary is a SCUT student}$$
> 三句话**结构完全相同**，只有主语不同。但命题逻辑只能给它们三个互不相关的字母，**"是华工学生"这个共同的部分被彻底丢掉了**。

> [!important] Limitation 2：**最基本的三段论都推不动**
> **情形 A**：已知 $P$: "Every student in SCUT is clever"，$Q$: "Peter is a SCUT student"。
> 我们凭常识知道能推出 "Peter is clever"，**但命题逻辑没有任何规则能做这一步**。
> **情形 B**：已知 $Q$: "Peter is a SCUT student"，$P$: "Peter cannot pass this Discrete Maths subject"。
> 常识能推出 "At least one student in SCUT cannot pass this Discrete Maths subject"，**命题逻辑同样无能为力**。
>
> 课件的结论（slide 10，红字）：
> **"Propositional Logic does not adequately express the following meanings: Every, all, some, partial, at least one, one, etc."**
> **"A more powerful tool, Quantifiers, will be introduced."**

> [!note] 停一下，这两个局限分别通向什么
> - **Limitation 1 → 谓词 Predicate**：把"是华工学生"抽出来做成 $S(x)$，主语变成参数。
> - **Limitation 2 → 量词 Quantifier**：把"每一个""至少一个"变成可以参与推理的符号 $\forall$、$\exists$。
>
> 所以这一节的两个主角，**各自解决一个具体的痛点**——这比"表达力不够"这句空话好记得多。

---

## L2. 课件强调的记号约定与"量化四要素"（Part1 slide 5、12）

> [!important] 记号约定（课件用红框标出）
> - **小写字母 (lowercase) 表示对象 objects**：$x, y, a, b$；
> - **大写字母 (UPPERCASE) 表示谓词 predicates**：$P, Q, S$。
>
> 课件还画了一张分解图：$P(x)$ 中 $P$ 是**谓词**，$x$ 是**变量**，箭头表示 $x$ **refer to**（指代）某个对象。
> **"命题函数的真值只有在变量的值确定之后才能确定。"**

> [!important] ⭐ 量化的**四个要素**（课件 slide 12，答题必写）
> 课件要求：**每写一个量化表达式，都要能说清这四样东西**。
> 1. **Quantifier 量词**（e.g. all, some）
> 2. **Variable 变量**
> 3. **Predicate 谓词**
> 4. **Domain 论域**
>
> 课件的例子：$P(x)$: "$x$ has a Huawei Mate 60"；"For all $x$, $P(x)$ is true"；"The domain consists of all students in this class"。
>
> 课件还给出了这门学问的正式名字：**处理谓词与量词的这块逻辑叫 Predicate Calculus（谓词演算）**。

> [!note] 这个"四要素"清单的实战价值
> 后面所有翻译题（L9、教材 §8）的标准答题格式就是它：
> **先写"量词 / 变量 / 论域 / 命题函数"四行，最后写 Answer。** 课件里每一道 Small Exercise 都是这么排版的——**照抄这个格式，步骤分不会丢**。

---

## L3. 判定量化命题真假的**四格表**（Part1 slide 22）⭐

课件用一张表把"怎么证 / 怎么否证"讲透了，**比教材的文字描述好用得多**：
...
>
> 课件在旁边用两个色块标出了**工作量的差别**：
> - **Finding one is ok（找到一个就行）**：证 $\exists$ 为真、证 $\forall$ 为假 —— 这叫 **counterexample 反例**；
> - **Need to consider ALL（必须考虑全部）**：证 $\forall$ 为真、证 $\exists$ 为假。

> [!important] ⭐ 这张表是本节最实用的一张
> **对角线关系**：$\forall$ 的"假"和 $\exists$ 的"真"都只要**一个例子**；$\forall$ 的"真"和 $\exists$ 的"假"都要**全部检查**。
> 考试时先判断题目问的是哪一格，再决定"举例子"还是"写通用论证"——**方向选错，整道题就废了**。

**课件的配套例子（slide 21）**：$P(x)$: $x^2<10$，论域是不超过 4 的正整数。
$$\forall x\,P(x)\equiv P(1)\wedge P(2)\wedge P(3)\wedge P(4)\equiv \mathbf F \qquad(\text{因为 }P(4)\text{ 假，}4\text{ 就是反例})$$
$$\exists x\,P(x)\equiv P(1)\vee P(2)\vee P(3)\vee P(4)\equiv\mathbf T$$

---

## L4. 量词优先级：课件的表与配套练习（Part1 slide 23–24、26–28）

> [!important] 课件的优先级表（沿用 Ch1.1 的表，再加一条）
> | Precedence | Operator | |
> |:-:|:-:|---|
> | 1 | $\neg$ | NOT |
> | 2 | $\wedge$ | AND |
...
>
> **红色大箭头标注：$\forall$ 和 $\exists$ 的优先级高于命题演算中的所有逻辑运算符。**
>
> **课件的判断题**：$\forall x\,P(x)\wedge Q(x)$ 该怎么理解？
> $$(\forall x\,P(x))\wedge Q(x)\quad\checkmark\qquad\qquad \forall x\,(P(x)\wedge Q(x))\quad\times$$

> [!example] 课件的 Small Exercise：给作用域加括号（slide 24）
> 原式：$\forall x\,(P(x)\wedge\exists z\,Q(x,z)\to\exists y\,R(x,y))\vee Q(x,y)$
>
> 课件**分三步**把括号逐层补出来：
> 1. $\forall x\,(\,P(x)\wedge(\exists z\,Q(x,z))\to(\exists y\,R(x,y))\,)\vee Q(x,y)$
> 2. $\forall x\,(\,(P(x)\wedge(\exists z\,Q(x,z)))\to(\exists y\,R(x,y))\,)\vee Q(x,y)$
>
> **答案（slide 27）**：
> - $\exists z$ 的作用域：$Q(x,z)$
> - $\exists y$ 的作用域：$R(x,y)$
...
> [!example] 课件的三道辨析题（slide 28）
> | 表达式 | 课件的判定 |
> |---|---|
> | $\forall x\,(\exists x\,P(x))$ | **$x$ 在 $\exists x P(x)$ 里已经不是自由变元**，所以外层的 $\forall x$ 绑定不到任何东西，**这个绑定是多余的** |
> | $(\forall x\,P(x))\wedge Q(x)$ | 第一个 $x$ 是**约束变元**，第二个 $x$ 是**自由变元**（它在 $\forall x$ 的作用域之外） |
> | $(\forall x\,P(x))\wedge(\exists x\,Q(x))$ | **合法**，因为这是**两个不同的 $x$**（各自被各自的量词绑定，互不干扰） |

---

## L5. ⭐⭐ 自由变元 vs 约束变元：课件的**七行对照表**（Part2 slide 2）

**这张表教材上完全没有**，是课件最有价值的原创内容之一。

> [!important] 课件原表（Recall….）
> | Feature 特征 | Free Variable 自由变元 | Bound Variable 约束变元 |
> |---|---|---|
> | **Definition 定义** | 不被任何量词约束 | 被量词（$\forall$、$\exists$）约束 |
> | **Analogy 类比** | 一个**没有明确先行词的代词** | 一个**有明确指代对象的代词** |
> | **Meaning 含义** | 不确定，**依赖上下文** | 由管辖它的量词决定 |
> | **Formula State 式子的状态** | 构成**命题函数**，而非命题 | 促成一个**命题**的形成 |
> | **Truth Value 真值** | **没有内在真值**，取决于变量赋值 | 该命题有**确定的真值** |
> | **Renaming 改名** | **不能**随意改名（会改变含义） | **可以**改名（只要保持一致） |
> | **Example 例子** | $x > 5$ 中的 $x$ | $\forall x\,(x>5)$ 中的 $x$ |

> [!important] 这张表里最该记住的两行
> **① "代词"类比**：自由变元像英语里的 "he"——不知道指谁；约束变元像 "every student"——指代明确。
> **② 改名规则**：$\forall x\,(x>5)$ 和 $\forall y\,(y>5)$ **是同一个命题**（约束变元可改名）；但 $x>5$ 和 $y>5$ 在同一段上下文里**不是同一回事**（自由变元不能乱改）。
>
> **编程类比**：约束变元 = 函数的**形参**（改名不影响函数行为），自由变元 = **全局变量 / 未定义的名字**（改名会出错）。

---

## L6. ⭐⭐ 量词分配律：课件用**四人真值表**举反例（Part2 slide 5–9）

教材只用文字说"哪两条成立、哪两条不成立"。课件对**每一条**都给了一张四个人的核对表——**这是本节最容易被考、也最容易记混的地方**。

> [!important] 课件的结论（slide 9 汇总）
> **全称量词：**
> $$\boxed{\forall x\,(P(x)\wedge Q(x))\equiv\forall x\,P(x)\wedge\forall x\,Q(x)}\quad\text{（成立，双向）}$$
...
> $$\boxed{\exists x\,(P(x)\vee Q(x))\equiv\exists x\,P(x)\vee\exists x\,Q(x)}\quad\text{（成立，双向）}$$

> [!example] 课件的反例表（$P(x)$: $x$ is lazy，$Q(x)$: $x$ likes beer）
> **反例一（slide 6）：$\forall x(P(x)\vee Q(x))\ \nrightarrow\ \forall x P(x)\vee\forall x Q(x)$**
> | | Peter | John | Mary | Jessica |
> |---|:-:|:-:|:-:|:-:|
> | $P(x)$: 懒 | ✗ | ✗ | ✗ | ✓ |
> | $Q(x)$: 喜欢啤酒 | ✓ | ✓ | ✓ | ✗ |
>
> 每个人**至少满足一条**，所以 $\forall x(P(x)\vee Q(x))$ **真**；
> 但"所有人都懒"假、"所有人都喜欢啤酒"也假，所以 $\forall xP(x)\vee\forall xQ(x)$ **假**。✗
>
> **反例二（slide 7）：$\exists xP(x)\wedge\exists xQ(x)\ \nrightarrow\ \exists x(P(x)\wedge Q(x))$**
> | | Peter | John | Mary | Jessica |
> |---|:-:|:-:|:-:|:-:|
> | $P(x)$: 懒 | ✓ | ✗ | ✗ | ✗ |
> | $Q(x)$: 喜欢啤酒 | ✗ | ✓ | ✗ | ✗ |
...
> [!important] ⭐ 一句话记住它
> **失败的两条，失败的原因是同一个：$\forall$ 和 $\exists$ 里的 $x$ 可以是不同的人。**
> - $\forall xP\vee\forall xQ$ 要求"**同一批人**全懒"或"**同一批人**全爱啤酒"，比"每人各占一样"强得多；
> - $\exists xP\wedge\exists xQ$ 允许懒的和爱啤酒的**是两个人**，比"存在一个人**同时**两样"弱得多。
>
> **速记：$\forall$ 配 $\wedge$、$\exists$ 配 $\vee$ 才能自由出入括号**——和受限论域里"$\forall$ 配 $\to$、$\exists$ 配 $\wedge$"是两条不同的规则，别混。

---

## L7. ⭐ 含"常量 $A$"的量词等价式（Part2 slide 10）

这组式子教材没有单列，但**化简量词表达式时天天要用**。

> [!important] 课件原表（**前提：$A$ 中不含自由变元 $x$**）
> | 等价式 |
> |---|
> | $\forall x\,(A\wedge P(x))\equiv A\wedge\forall x\,P(x)$ |
> | $\forall x\,(A\vee P(x))\equiv A\vee\forall x\,P(x)$ |
> | $\exists x\,(A\wedge P(x))\equiv A\wedge\exists x\,P(x)$ |
> | $\exists x\,(A\vee P(x))\equiv A\vee\exists x\,P(x)$ |
> | ⭐ $\forall x\,P(x)\to A\equiv\exists x\,(P(x)\to A)$ |
> | ⭐ $A\to\forall x\,P(x)\equiv\forall x\,(A\to P(x))$ |
> | ⭐ $\exists x\,P(x)\to A\equiv\forall x\,(P(x)\to A)$ |
> | ⭐ $A\to\exists x\,P(x)\equiv\exists x\,(A\to P(x))$ |
>
> **课件用红字特别注明：$A$ does not consist of free variable $x$（$A$ 里不能含自由的 $x$）。**

> [!example] 课件给出的两条推导（slide 10 右侧的红框与绿框）
> **红框：$\forall x\,P(x)\to A\equiv\exists x\,(P(x)\to A)$**
> $$\begin{aligned}
> \forall x\,P(x)\to A &\equiv\neg(\forall x\,P(x))\vee A && \text{消箭头}\\
> &\equiv\exists x\,(\neg P(x))\vee A && \text{量词德摩根律}\\
> &\equiv\exists x\,(\neg P(x)\vee A) && \text{把 }A\text{ 移进去（}A\text{ 不含 }x\text{）}\\
> &\equiv\exists x\,(P(x)\to A) && \text{装回箭头}
> \end{aligned}$$
>
> **绿框：$A\to\forall x\,P(x)\equiv\forall x\,(A\to P(x))$**
> $$A\to\forall x\,P(x)\equiv\neg A\vee\forall x\,P(x)\equiv\forall x\,(\neg A\vee P(x))\equiv\forall x\,(A\to P(x))$$

> [!warning] ⭐ 最容易记反的一条
> $$\exists x\,P(x)\to A\equiv\forall x\,(P(x)\to A)\qquad\text{（}\exists\text{ 进入箭头左边会变成 }\forall\text{）}$$
> **原因**：箭头左边相当于被否定了一次，而否定会把 $\exists$ 翻成 $\forall$。
> 这和 [[C1.3 命题等价 Propositional Equivalences]] 里 $(p\to r)\wedge(q\to r)\equiv(p\vee q)\to r$ 的"前件联结词翻转"**是同一个机制**。
>
> **课件在 Ch1.5 Part2 slide 12 还给了一个中文直觉版**：
> $\forall x(P(x)\to A)$ 读作"**对每个人，只要他努力，中国就会强大**"；$\forall x P(x)\to A$ 读作"**如果所有人都努力，中国才会强大**"——后者弱得多，两者**不等价**。

---

## L8. 受限论域：课件给的**三种写法**（Part2 slide 24）

教材只给"简写记法"。课件把同一句话用**三种方式**都写了一遍，**考试三种都可能要求**。

> [!important] 例：**"The square of every real number greater than 10 is greater than 100."**
> | 写法 | 表达式 | 论域 |
> |---|---|---|
> | **Using Domain**（把限制写进论域） | $\forall x\,(x^2>100)$ | 论域 = **大于 10 的实数** |
> | **Using Predicate**（把限制写成谓词） | $\forall x\,(x>10\to x^2>100)$ | 论域 = **全体实数** |
> | **Using Abbreviated Notation**（简写记法） | $\forall x>10\,(x^2>100)$ | 论域 = **全体实数** |
>
> 三者**完全等价**，只是把"大于 10"这个限制放在了不同的位置。

> [!example] 课件的配套读法练习（slide 25）
> 论域为实数时：
> - $\forall x<0\,(x^2>0)$ → "**负实数的平方是正的**"
> - $\forall y\ne0\,(y^3\ne0)$ → "**非零实数的立方是非零的**"
> - $\exists z>0\,(z^2=2)$ → "**2 有一个正的平方根**"

---

## L9. 翻译题：课件的"**换论域就换写法**"对照（Part2 slide 16–23）

课件对**同一句话**反复演示"选不同论域会得到不同表达式"，这正是教材 §8 的核心，但课件的排版更适合照抄。

> [!important] 课件反复强调的两句话（红框标出）
...
> [!example] 例一：**"Every student in this class is lazy."**
> **论域 = 本班学生**：$P(x)$: $x$ is lazy　⇒　$\forall x\,P(x)$
> **论域 = 任何人**：$Q(x)$: $x$ 是本班学生，$P(x)$: $x$ is lazy
> $$\forall x\,(Q(x)\to P(x))\quad\checkmark\qquad\qquad\forall x\,(Q(x)\wedge P(x))\quad\times$$
> 课件的说明：前者是"对每个人，**如果**他在本班，他就懒"；后者是"**每个人都既在本班又懒**"——把全世界的人都拉进本班了。

> [!example] 例二：**"Some student in this class is lazy."**
> **论域 = 本班学生**：$\exists x\,P(x)$
> **论域 = 任何人**：
> $$\exists x\,(Q(x)\wedge P(x))\quad\checkmark\qquad\qquad\exists x\,(Q(x)\to P(x))\quad\times$$
> 课件的说明（红框）：错误写法 **"including the case which contains no person in this class"**——只要存在一个**不在本班**的人，$Q(x)$ 为假就让整个蕴涵为真，**命题被平凡地满足了**。

> [!example] 例三（课件 Small Exercise，slide 20–23）：IBM 员工
> **"Every staff in IBM company has visited Mexico."**
> | 解法 | 论域 | 表达式 |
> |---|---|---|
> | Solution 1 | **IBM 员工** | $\forall x\,P(x)$，$P(x)$: $x$ 去过墨西哥 |
...
> **这是建模的通用原则——能参数化的就不要枚举**，和写代码时"用参数而不是复制粘贴函数"完全一致。

> [!example] 例四（课件 Small Exercise，slide 26–28）：系统规约两题
> 这两题**与教材例 25 完全相同**，见本笔记 §9。课件给的答案：
> - "Every mail message larger than one megabyte will be compressed."
> $$\forall m\,\bigl(S(m,1)\to C(m)\bigr)$$
> 其中 $S(m,y)$: "邮件 $m$ 大于 $y$ 兆字节"，$C(m)$: "邮件 $m$ 会被压缩"。
> - "If a user is active, at least one network link will be available."
> $$\exists u\,A(u)\to\exists n\,S(n,\text{available})$$
> 其中 $A(u)$: "用户 $u$ 是活跃的"，$S(n,x)$: "网络链路 $n$ 处于状态 $x$"。
>
> **注意第二题的量词位置**：$\exists u\,A(u)$ 整体作为前件，**不能**写成 $\exists u\,(A(u)\to\cdots)$——这正是 L7 里 $\exists x P(x)\to A$ 那条等价式管的情形。
...
> [!info] 来源
> 本节内容来自课堂录音转写稿 `transcript/1-8节课.txt`（第 5、6 节课）。
...
> [!important] 老师的定位
> **"离散数学更像一座桥梁，连接人类的自然语言和计算机科学的编程语言。"**
> 而学到 §1.4，这座桥要往前修一步：**不只看句子之间的关系，还要看句子内部的结构**。
>
> 递进关系（课堂原话整理）：
> $$\underbrace{\text{命题}}_{\text{句子是真是假}}\ \longrightarrow\ \underbrace{\text{谓词}}_{\text{句子之间的\textbf{共性}}}\ \longrightarrow\ \underbrace{\text{量词}}_{\text{共性涉及\textbf{多少个}}}$$
> - **命题不够用**：$p,q,r$ 三个字母看不出"都是华工学生"这个共同点；
> - **谓词补上共性**：抽出 $S(x)$ = "$x$ 是华工学生"；
> - **量词补上数量**："**至少有一个**学生挂科"——挂了几个不知道，但"至少一个"这件事必须能写出来。
>
> **"一层一层递进，才能把信息从自然语言里抽象出来。"**

> [!note] 论域的例子（课堂原创，课件上只有插图）
> $P(x)$: "$x$ breathes oxygen"。
> - 论域 = **人类** ⇒ $\forall x\,P(x)$ **真**；
> - 论域 = **生物 creatures** ⇒ **假**——老师提示：**高中生物学过，有些细菌不需要氧气**；
> - 论域 = **包括外星人** ⇒ 更假。老师拿电影《**挽救计划 / Project Hail Mary**》里那个"**需要氨气而不是氧气**"的岩石外星人 Rocky 举例（他还当场和同学确认了一下片名）。
>
> **要点：同一个谓词，换论域就换真值。** 写量化命题前**先问自己论域是什么**。

---

## T2. 自由变元 / 约束变元的那张表是怎么来的

> [!note] 它是为了回应学生提问才做的
> 第 6 节课一开始老师就说：
> **"有些同学对 free variable 和 bound variable 还有困惑，所以我做了一张表来总结这两个新定义的区别。"**
>
> 也就是说，[[C1.4 谓词与量词 Predicates and Quantifiers]] 课件补充 L5 的**七行对照表是老师专门加的**，不在原教材里——**属于高概率考点**。

> [!example] ⭐ 课堂上重点讲的那个例子
> $$\forall x\,P(x,y)$$
> - **$x$ 是约束变元**（被 $\forall x$ 绑定）；
> - **$y$ 是自由变元**——老师特别强调：**"请注意只有 $x$ 是约束变元，$y$ 是自由的，因为 $y$ 只出现在函数内部，而不是整个命题函数里。"**
> - 所以**整个式子仍然不是命题**，因为 $y$ 没有确定。
>
> **判断口诀：一个式子里只要还剩一个自由变元，它就还不是命题。**

---

## T3. 量词分配律：课堂对"为什么不成立"的解释

> [!important] ⭐ 学生晚上提问、老师第二天专门回答的一条
> 老师提到："**昨天晚上有同学问我，为什么这两个 $x$ 是不同的。**"
>
> 针对 $\exists x\,P(x)\wedge\exists x\,Q(x)\ \nrightarrow\ \exists x\,(P(x)\wedge Q(x))$，他的中文解释是：
> **"这四个人里面很懒惰的那个人，和这四个人里面很喜欢喝啤酒的那个人，未必是同一个人。
> 但右边（$\exists x(P\wedge Q)$）要求的是：存在这么一个人，他既喜欢喝啤酒、同时又很懒惰。"**
>
> **一句话：$\exists$ 各写各的时，两个 $x$ 可以是两个人；写在一个 $\exists$ 里，就必须是同一个人。**

---

## T4. 翻译题：$\to$ 与 $\wedge$ 的差别，课堂上讲得比课件透

> [!important] 为什么全称量词配 $\to$ 而不是 $\wedge$
> 课件只说"universal quantifier connects with an implication"。课堂上老师解释了**原因**：
>
> - $\forall x\,(Q(x)\to P(x))$：**"对每个人，如果他在这个班里，他就很懒。"**
> - $\forall x\,(Q(x)\wedge P(x))$：**"每个人都既在这个班里、又很懒。"**——把全世界的人都拉进本班了。
>
> **老师给的关键词是"递进关系"**：
> **"蕴涵符号 $\to$ 体现的是不同层次——第一步你要先在这个课堂里，第二步才轮到说你懒不懒。
> 而 $\wedge$ 把两个谓词放在了同一个地位。"**
>
> **换句话说：$\to$ 先划范围、再下判断；$\wedge$ 是并排陈述。** 这就是为什么受限论域必须配 $\to$。

> [!note] 一句话里可以有好几个谓词
> 老师专门澄清了一个学生常见的疑惑：
> **"我没说一个自然语言句子只能有一个谓词，它可以包含好几个谓词。"**
> 例如"本班的每个学生都很懒"在"任何人"论域下就需要 $Q(x)$（是本班学生）和 $P(x)$（很懒）**两个**谓词。
