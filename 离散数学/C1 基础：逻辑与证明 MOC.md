...
| **办公室** | **B1-C513**（**需先发邮件预约**，课堂明确说明不接受随机敲门） |
...
| 项目                                   |        占比         | 说明                                                         |
| ------------------------------------ | :---------------: | ---------------------------------------------------------- |
| **出勤与参与 Attendance & Participation** |      **2%**       | 课堂会随机点名提问，**回答一次即可拿满**，**答对答错都算分**。不回答就拿不到。课堂说明：学院要求必须有考勤分 |
...
> 课堂给的原则：**“你如果是 59 分差那 1 分到 60，我不会刁难你；但你 89 分想要 90，那就不会帮。”**
...
>   课堂原话：“9 月 24 日之前我们会开始讲第二章和其他章节，**但第二章不需要为期中复习**”——因为两天挨得太近。
...
>
> **备考动作**：对照 syllabus 圈出讲过的小节 → **刷 Rosen 对应小节的课后习题** → **用教材最后几页的答案自查**（课堂特别提到：textbook 最后几页会列出最终答案）。
...
> - **记不住名字，把该定律的等式写在旁边同样给分**——课堂原话：“摩根肯定是人名，我都记不住。你可以写公式，我就知道你是什么意思。”
...
> 详见 [[C1.6 推理规则 Rules of Inference]] 正文（🎤 标记处）。

> [!warning] $\mathbf T$ / $\mathbf F$ 禁止当变量用
> 在离散数学里 $\mathbf T$、$\mathbf F$ 有特殊含义（恒真/恒假），**不能拿来代表命题变元**。
> 课堂在 §1.3 和 §1.6 各强调过一次，而且**当场承认课件上误用了 $T$ 做代换变量**，说“考试这么写可能要扣分”。
> **做代换时请用 $S,R$ 或 $A,B$。**
...
>
> ⚠️ 课堂专门讲过一个案例：上学期有同学称已提交但系统未收到，**直到期末合分时才反映**，课堂上明确说这不合理。
...
> - 同一位任课教师的强化学习课上抓到有学生**用大模型拍照答题**，该课直接挂科。
>
...
> | 课件文件 | 日期 | 覆盖内容 | 已并入笔记 |
> |---|---|---|---|
> | `Ch1.1_Part1.pdf`（23 slides） | 2026-09-02 | Warm Up 四题、Wason 随堂测四题、命题定义、$\neg\ \wedge\ \vee\ \oplus$、两道 Small Exercise | [[C1.1 命题逻辑 Propositional Logic]] 正文（📚 标记处） |
> | `Ch1.1_Part2.pdf`（20 slides） | 2026-09-02 | 真值表、条件语句、必要/充分条件、双条件、运算符总表 | [[C1.1 命题逻辑 Propositional Logic]] 正文（📚 标记处） |
> | `Ch1.1_Part3.pdf`（18 slides） | 2026-09-03 | 复合命题、**运算优先级**、加括号五步、逐列建表、翻译四步算法 | [[C1.1 命题逻辑 Propositional Logic]] 正文（📚 标记处） |
> | `Ch1.2.pdf`（12 slides） | 2026-09-03 | 系统规约（真值表解法）、位运算、Batman/Joker 谜题、**命题类型** | [[C1.2 命题逻辑的应用 Applications of Propositional Logic]] 正文（📚 标记处） |
> | `Ch1.3.pdf`（29 slides） | 2026-09-08 | 逻辑等价定义、真值表法及其代价、Table 6 逐条真值表验证、德摩根律推广、**Table 7/8 全部推导** | [[C1.3 命题等价 Propositional Equivalences]] 正文（📚 标记处） |
> | `Ch1.4_Part1.pdf`（29 slides） | 2026-09-09 | 命题逻辑的两个局限、谓词、量化四要素、三类量化、判真判假四格表、量词优先级、作用域与自由/约束变元 | [[C1.4 谓词与量词 Predicates and Quantifiers]] 正文（📚 标记处） |
> | `Ch1.4_Part2.pdf`（29 slides） | 2026-09-10 | **自由/约束变元七行对照表**、**量词分配律的四人反例表**、**含常量 $A$ 的八条等价式**、量词德摩根律、受限论域三种写法、翻译与系统规约 | [[C1.4 谓词与量词 Predicates and Quantifiers]] 正文（📚 标记处） |
> | `Ch1.5_Part1.pdf`（13 slides） | 2026-09-10 | 嵌套量词判读口诀、量词顺序、$xy=0$ 随堂测、翻译例题 | [[C1.5 嵌套量词 Nested Quantifiers]] 正文（📚 标记处） |
> | `Ch1.5_Part2.pdf`（22 slides） | 2026-09-15 | ⭐ **「恰好一个」四种情形（15 张）**、$\forall x(P(x)\to A)\equiv\exists xP(x)\to A$、有限论域展开、嵌套量词的否定 | [[C1.5 嵌套量词 Nested Quantifiers]] 正文（📚 标记处） |
> | `Ch1.6_Part1.pdf`（8 slides） | 2026-09-15 | 两个错误论证引入、论证/前提/结论、有效性判据（“只盯前真后假”） | [[C1.6 推理规则 Rules of Inference]] 正文（📚 标记处） |
> | `Ch1.6_Part2.pdf`（33 slides） | 2026-09-16 | Table 1 八条规则、**推理 vs 等价对照表**、**⇒ 链式书写法**、两个谬误、四条量化规则、全称肯定前件式 | [[C1.6 推理规则 Rules of Inference]] 正文（📚 标记处） |
> | `Ch1.6_Part3.pdf`（20 slides） | 2026-09-16 | ⚠️ **文件名写 1.6，内容是教材 §1.7**：非形式证明、定理与证明的分类表、直接证明（含“死胡同”例）、逆否证明 | [[C1.7 证明导论 Introduction to Proofs]] 正文（📚 标记处） |
...
> [[C1.1 命题逻辑 Propositional Logic]]、[[C1.3 命题等价 Propositional Equivalences]]、[[C1.4 谓词与量词 Predicates and Quantifiers]]、[[C1.5 嵌套量词 Nested Quantifiers]]、[[C1.6 推理规则 Rules of Inference]]、[[C1.7 证明导论 Introduction to Proofs]] 各篇**正文中的 🎤 / 📚 / 🔧 标记处**（已按知识点就地并入对应小节，不再单独成章），以及本页的「🎓 课程信息与考试情报」。
...
> 唯一需要说明的是 [[C1.7 证明导论 Introduction to Proofs]] §8 反证法——**第 1 节课**的「证明方法概览」里口头介绍过它的做法（“假设结论不成立，倒推出矛盾”），在**第 8 节课**的分类表里也点了名，但**始终没有展开、没有给例子**，故维持「课堂仅点到方法名，展开为拓展」的写法。
...
| §1.7 “恰好一个”的表达 | **提前到 §1.5** | ⭐ 课件 Ch1.5 Part2 用 15 张幻灯片专讲 |
| **§1.8 全节** | **暂未讲** | 整节未开讲 |
...
> [!warning] 只有一处，但很关键
> **运算符优先级**：教材 Table 8 只列 5 个运算符（不含 $\oplus$）；**课件的表把 $\vee$ 和 $\oplus$ 并列为第 3 级**：
>
> | 优先级 | 运算符 |
> |:-:|:-:|
> | 1 | $\neg$ |
...
> | 5 | $\leftrightarrow$ |
>
> 详见 [[C1.1 命题逻辑 Propositional Logic]] 正文（📚 标记处）。
...
> | 内容 | 位置 | 为什么重要 |
> |---|---|---|
> | **Table 7/8 全部现场推导，只背 $P\to Q\equiv\neg P\vee Q$** | [[C1.3 命题等价 Propositional Equivalences]] 正文（📚 标记处） | ⭐⭐ 老师不要求背表，**要求会推**——考法完全不同 |
> | **自由 / 约束变元七行对照表** | [[C1.4 谓词与量词 Predicates and Quantifiers]] 正文（📚 标记处） | ⭐ 概念辨析题的标准答案来源 |
> | **量词分配律的四人反例表** | [[C1.4 谓词与量词 Predicates and Quantifiers]] 正文（📚 标记处） | ⭐⭐ 两条不成立的分配律，必须会举反例 |
> | **含常量 $A$ 的八条量词等价式** | [[C1.4 谓词与量词 Predicates and Quantifiers]] 正文（📚 标记处） | ⭐ 教材未列，化简量词式必用 |
> | **「恰好一个」四种情形 + 两个陷阱** | [[C1.5 嵌套量词 Nested Quantifiers]] 正文（📚 标记处） | ⭐⭐ 课堂占时最多，翻译大题的头号考点 |
> | **推理 vs 等价对照表、⇒ 链式书写法** | [[C1.6 推理规则 Rules of Inference]] 正文（📚 标记处） | ⭐ 澄清 $\to$ 与 $\leftrightarrow$ 的分工；第二种答题格式 |
...
>
> > [!success]- 参考答案
> > **a)** "$q$ only if $p$" = $\boxed{q \to p}$
> > **b)** $\boxed{q \wedge p}$
> > **c)** $\boxed{\neg q \vee \neg p}$（等价于 $\neg(p \wedge q)$，德摩根律）
> > **d)** “充要条件” ⇒ $\boxed{q \leftrightarrow p}$
...
> **题目**：$3x+1$ 迭代规则——$x$ 为偶数时取 $x/2$，$x$ 为奇数时取 $3x+1$。
> 39. 对 a) $x=6$　b) $x=7$　c) $x=17$　d) $x=21$ 写出迭代轨迹，验证最终到达 1。
> 40. 对 a) $x=16$　b) $x=11$ 做同样的事。
>
> > [!success]- 参考答案
...
> 13. 受限论域下 $\forall$ 配什么？$\exists$ 配什么？
> 14. $\neg\forall xP(x)$ 等于？
> 15. $\neg\forall x(P(x)\to Q(x))$ 等于？
> 16. 哪些量词分配律成立？
> 17. 空论域下 $\forall$ 和 $\exists$ 的真值？
> 18. $\forall x\exists y$ 与 $\exists y\forall x$ 谁强？
> 19. 写出极限的量词定义。
> 20. $\exists!xP(x)$ 展开？
>
> > [!success]- 答案
> > 13. **$\forall$ 配 $\to$，$\exists$ 配 $\wedge$**
> > 14. $\exists x\neg P(x)$
> > 15. $\exists x(P(x)\wedge\neg Q(x))$（**反例**）
> > 16. **$\forall$ 对 $\wedge$、$\exists$ 对 $\vee$**（另两个不成立）
> > 17. $\forall$ 恒真，$\exists$ 恒假
> > 18. **$\exists y\forall x$ 强**，单向蕴涵 $\exists\forall \Rightarrow \forall\exists$
> > 19. $\forall\varepsilon>0\ \exists\delta>0\ \forall x(0<|x-a|<\delta \to |f(x)-L|<\varepsilon)$
> > 20. $\exists x(P(x) \wedge \forall y(P(y) \to y = x))$

> [!question] 推理与证明（1.6–1.8）
> 21. 有效 (valid) 与可靠 (sound) 的区别？
> 22. 两个谬误是什么？
> 23. UG 对 $c$ 有什么要求？
> 24. 一个证明中 EI 和 UI 谁先用？
> 25. 空证明与平凡证明的依据？
> 26. 反证法证 $p\to q$ 假设什么？
> 27. 证 $n$ 个命题等价最省力的方法？
> 28. 逆向推理的正确方向？
...
>
> > [!success]- 答案
> > 21. 有效只保证形式；可靠 = 有效 + 前提全真
> > 22. **肯定后件**（$p\to q, q \therefore p$）、**否定前件**（$p\to q,\neg p\therefore\neg q$）
> > 23. 必须是**任意**元素，不能用它的特殊性质
> > 24. **EI 先**
> > 25. 前提为假 / 结论为真
> > 26. **$p$ 且 $\neg q$**
> > 27. **循环蕴涵**，$n$ 个条件语句而非 $n^2-n$ 个
> > 28. 找可证的 $p$ 使 $p \to q$（**反了就是窃取论题**）
> > 29. **等于每块拼图覆盖的格子数**
> > 30. **不存在能证明所有数学定理的程序**（哥德尔 / 丘奇—图灵）
