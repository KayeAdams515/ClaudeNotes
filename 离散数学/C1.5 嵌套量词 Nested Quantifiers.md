...
> 课件独有的**「恰好一个 Exactly One」四种情形**（整整 15 张幻灯片）见文末「📚 课件补充」，这是本节课堂上花时间最多的部分。
...
# 📚 课件补充 Lecture Slides（Dr. 欧士琪 Ch1.5 Part 1 & 2）

> [!info] 课件信息
> **Discrete Mathematics · Chapter 1: Logic and Proof · 1.5 Nested Quantifiers**
> 主讲：**Dr Shiqi (Shawn) Ou, 欧士琪**，South China University of Technology
> **Part 1**：2026 年 9 月 10 日，13 张　|　**Part 2**：2026 年 9 月 15 日，22 张
>
> **Part 2 的 15 张幻灯片（slide 3–17）全部在讲同一件事：怎么用 $\forall$ 和 $\exists$ 表达"恰好一个"。**
> 这是整节课的重心，也是教材上**没有系统展开**的内容。

---

## L1. 课件的判读口诀（Part1 slide 3）

> [!important] 课件原文
> - 两个量词**嵌套 nested**，指的是一个在另一个的**作用域内**。
> - **If quantifiers are the same type, the order is not a matter.**（同类量词，顺序无所谓。）
> $$\exists x\,\exists y\,(x+y=0)\quad\text{与}\quad\exists y\,\exists x\,(x+y=0)\qquad\text{Same meaning}$$
> - **If quantifiers are different types, read from left to right.**（不同类量词，**从左往右读**。）
> $$\forall x\,\exists y\,(x+y=0)\quad\text{与}\quad\exists y\,\forall x\,(x+y=0)\qquad\text{Different meaning}$$

> [!important] ⭐ "从左往右读"这五个字是本节唯一需要的技巧
> 课件把它翻译成一个固定句式：
...
> **逐个量词按顺序念出来，念完自然就知道是什么意思**——不要试图整体"意会"。

**课件的两组对照**（Part1 slide 4–5）：

| 表达式 | 课件的英文读法 | 中文 | 真假（论域=实数） |
|---|---|---|:-:|
| $\forall x\exists y\,L(x,y)$ | For all persons, there is a person they love | **所有人都爱着某个人** | —— |
| $\exists y\forall x\,L(x,y)$ | There is a person who is loved by all persons | **有个人被所有人爱** | —— |
| $\forall x\exists y\,(x+y=0)$ | 每个实数都有加法逆元 | ✓ | **真** |
| $\exists y\forall x\,(x+y=0)$ | 有一个实数是所有实数的加法逆元 | ✗ | **假** |

> [!example] 课件的课堂测验（Part1 slide 7）
> 论域为实数，$P(x,y)$: "$xy=0$"。下列哪些为真？
> | 选项 | 表达式 | 判定 | 理由 |
> |:-:|---|:-:|---|
...
>
> **这道题值得反复看**：C 和 D 都真，是因为 $0$ 这个元素**同时**扮演了"对每个 $x$ 都能配上的 $y$"和"对所有 $y$ 都管用的万能 $x$"。
> 一般情况下 $\exists\forall$ 比 $\forall\exists$ 强得多，**这里恰好两者都成立，是个特例，不是通例**。

---

## L2. ⭐⭐ 课件的重头戏：**"恰好一个 Exactly One"的四种情形**（Part2 slide 3–17）

> [!important] 课件的出发点（slide 3）
> **"It also called uniqueness quantification."** 教材记作 $\exists!x\,P(x)$ 或 $\exists_1 x\,P(x)$。
> **但课件说：我们要用 $\forall$ 和 $\exists$ 把"恰好一个"表达出来**（因为 $\exists!$ 不是基本量词，推理规则也不直接支持它）。
>
> 以下全部设 $L(x,y)$ 表示 "**$x$ loves $y$**"。**课件讨论了四种情形。**

### L2.1 ⭐ 通用模板：**"存在 + 唯一"两部分**

> [!important] 把"恰好一个"拆成两句话——**这是全部四种情形的共同结构**
> $$\underbrace{\exists x\,\bigl(\ \text{某某性质}(x)}_{\textbf{① 至少有一个}}\ \wedge\ \underbrace{\forall z\,\bigl(\text{某某性质}(z)\to z=x\bigr)}_{\textbf{② 至多有一个}}\ \bigr)$$
> **第一部分说"找得到"，第二部分说"找到的都是同一个"。**
> 课件把第二部分叫做"If anyone …, it must be $x$"——**"任何满足条件的都必须就是它"**。

### L2.2 情形一：**Mary loves exactly one person**（slide 4–6）

> [!important] 课件的两个版本
> **Version 1（用"不等则不爱"）**
> $$\exists x\,\bigl(L(\text{Mary},x)\wedge\forall z\,(z\ne x\to\neg L(\text{Mary},z))\bigr)$$
> 读法：存在一个 $x$，Mary 爱 $x$；且**任何不是 $x$ 的人 $z$，Mary 都不爱**。
>
> **Version 2（用"爱则相等"）**
> $$\exists x\,\bigl(L(\text{Mary},x)\wedge\forall z\,(L(\text{Mary},z)\to z=x)\bigr)$$
> 读法：存在一个 $x$，Mary 爱 $x$；且**Mary 爱的任何人 $z$ 都必定就是 $x$**。
>
> **课件明确指出：Version 1 与 Version 2 是同一件事**，因为
> $$\underbrace{z\ne x\to\neg L(\text{Mary},z)}_{\neg p\,\to\,\neg q}\qquad\text{与}\qquad\underbrace{L(\text{Mary},z)\to z=x}_{q\,\to\,p}$$
> **互为逆否命题**（$p\to q\equiv\neg q\to\neg p$，见 [[C1.3 命题等价 Propositional Equivalences]]）。

> [!important] ⭐ 考试写哪个版本？
> **写 Version 2。** 理由：
> 1. 它是"**至多一个**"的标准写法，和教材 $\exists!$ 的展开式完全一致；
> 2. 它只有一个否定符号都不用，**不容易写错**；
> 3. 后面三种情形课件全部用的是 Version 2 的形状。

### L2.3 四种情形的完整对照表 ⭐

> [!important] 课件 slide 4、7、8、9 的四个答案（**建议直接背下这张表的结构**）
>
> | 情形 | 中文 | 表达式 |
> |:-:|---|---|
...
>
> **Case 1 vs Case 2 的差别只在 $L$ 的两个参数谁在前**（谁爱谁）。
> **Case 3 是 Case 1 外面套一个 $\forall y$**（把 Mary 换成"每个人"）。
> **Case 4 最难**，下面单独讲。

### L2.4 ⭐ Case 4 的两个陷阱（slide 9–13）

**"Exactly one person loves all people."（恰好一个人爱所有人）**

> [!warning] 陷阱一：唯一性条件里必须写 $\forall w\,L(z,w)$，不能只写 $L(z,y)$
> **正确**：
> $$\exists x\,\forall y\,\bigl(L(x,y)\wedge\forall z\,(\ \boxed{\forall w\,L(z,w)}\ \to z=x)\bigr)$$
> **错误**：
> $$\exists x\,\forall y\,\bigl(L(x,y)\wedge\forall z\,(\ \boxed{L(z,y)}\ \to z=x)\bigr)$$
>
> **课件的解释（slide 13，配了一张 A、B、C 三点的关系图）**：
> - 写 $\forall w\,L(z,w)$ 的意思是"**如果 $z$ 爱所有人，那么 $z$ 就是 $x$**"——这才是"爱所有人的那个人是唯一的"。
> - 写 $L(z,y)$ 的意思是"**对任何 $y$ 和任何 $z$，只要 $z$ 爱 $y$，$z$ 就是 $x$**"——这等于说**全世界只有 $x$ 一个人会爱人**，太强了。
>
> **口诀：唯一性条件里的"性质"，必须和存在性部分的"性质"完全一样。**
> 存在部分说的是"$x$ 爱**所有**人"，唯一部分就必须说"爱**所有**人的那个 $z$ 等于 $x$"。

> [!example] 陷阱二：$\forall y$ 能不能提到括号外面？（slide 11）
> 课件做了一串等价变形，**结论是可以**：
> $$\begin{aligned}
> &\exists x\,\forall y\,\bigl(L(x,y)\wedge\forall z\,(\forall w\,L(z,w)\to z=x)\bigr)\\
> \Leftrightarrow\ &\exists x\,\bigl(\forall y\,L(x,y)\wedge\forall y\,\forall z\,(\cdots)\bigr) && \text{用 }\forall x(P\wedge Q)\equiv\forall xP\wedge\forall xQ\\
> \Leftrightarrow\ &\exists x\,\bigl(\forall y\,L(x,y)\wedge\forall z\,(\forall w\,L(z,w)\to z=x)\bigr) && \text{后半部分不含 }y\text{，}\forall y\text{ 可删}
> \end{aligned}$$
> **依据**：$\forall x\,(A\wedge P(x))\equiv A\wedge\forall x\,P(x)$（$A$ 不含 $x$）——正是 [[C1.4 谓词与量词 Predicates and Quantifiers]] 课件补充 L7 那组等价式。

> [!important] ⭐ 顺带学到的一条重要等价（slide 12）
> $$\boxed{\forall x\,(P(x)\to A)\equiv\exists x\,P(x)\to A}\qquad(A\text{ 中不含 }x)$$
> **课件给的中文直觉**（$P(x)$: "$x$ 努力工作"，$A$: "中国会强大"）：
> - $\forall x\,(P(x)\to A)$：**"对每个人来说，只要他努力，中国就会强大"**——任何一个人努力都够了；
> - $\forall x\,P(x)\to A$：**"如果所有人都努力，中国才会强大"**——要求高得多。
>
> **两者不等价**，前者等于 $\exists x\,P(x)\to A$。这是本节最容易搞反的一条。

### L2.5 课件的两道课堂练习（slide 16–17）

> [!example] 练习一：**"There is exactly one person whom everybody loves."**（恰好有一个人被所有人爱）
> **拆两步**：
> ① 存在性：有个人被所有人爱 ⇒ $\exists x\,\forall y\,L(y,x)$
> ② 唯一性：被所有人爱的那个人必须是 $x$ ⇒ $\forall z\,(\forall w\,L(w,z)\to z=x)$
> **答案**：
> $$\exists x\,\bigl(\forall y\,L(y,x)\wedge\forall z\,(\forall w\,L(w,z)\to z=x)\bigr)$$
> **注意**：和 Case 4 一样，唯一性里写的是 $\forall w\,L(w,z)$（被**所有人**爱），不是 $L(y,z)$。

> [!example] 练习二：⭐ **"Exactly two people love Mary."**（恰好两个人爱 Mary）
> **课件把"恰好两个"拆成三句话**：
> ① **至少两人**爱 Mary：$\exists x\,\exists y\,\bigl(L(x,\text{Mary})\wedge L(y,\text{Mary})\wedge x\ne y\bigr)$
> ② **至多两人**爱 Mary：任何爱 Mary 的人**要么是 $x$ 要么是 $y$**
> $$\forall z\,\bigl(L(z,\text{Mary})\to(z=x\vee z=y)\bigr)$$
> **完整答案**：
...
>
> **要点：$x\ne y$ 这个合取项不能漏！** 漏了就变成"至少一个"（$x$ 和 $y$ 可以是同一人）。
> **推广到"恰好 $n$ 个"**：$n$ 个存在量词 + $\binom n2$ 个两两不等 + 一个"任何满足者必属于这 $n$ 个之一"。

---

## L3. 量词展开成有限合取/析取：课件的 $3\times3$ 例子（Part2 slide 18–19）

> [!important] 课件的 Recall
> 论域元素可以一一列出（离散、有限）时：
> $$\forall x\,P(x)\equiv P(x_1)\wedge P(x_2)\wedge\cdots\wedge P(x_n)\qquad \exists x\,P(x)\equiv P(x_1)\vee P(x_2)\vee\cdots\vee P(x_n)$$

> [!example] 课件例题：把 $\forall x\,\exists y\,P(x,y)$ 展开（论域 = 不超过 3 的正整数）
> $$\begin{aligned}
> \forall x\,\exists y\,P(x,y)&\equiv\exists y\,P(1,y)\wedge\exists y\,P(2,y)\wedge\exists y\,P(3,y)\\
> &\equiv\bigl[P(1,1)\vee P(1,2)\vee P(1,3)\bigr]\wedge\\
> &\quad\ \bigl[P(2,1)\vee P(2,2)\vee P(2,3)\bigr]\wedge\\
> &\quad\ \bigl[P(3,1)\vee P(3,2)\vee P(3,3)\bigr]
> \end{aligned}$$
>
> **要点：先展开外层 $\forall$（得到三个合取项），再展开每个内层 $\exists$（各得三个析取项）。**
> **展开后的形状一眼就能看出量词顺序的含义**：
> - $\forall x\exists y$ 是"**合取套析取**"——每一行只要有一个真就行；
> - $\exists y\forall x$ 是"**析取套合取**"——必须有一整列全真。
> 这是理解量词顺序最直观的一个角度，比抽象地想"依赖关系"容易得多。

---

## L4. 嵌套量词的否定：课件的完整例子（Part2 slide 20–21）

> [!important] 课件的 Recall
> $$\neg\forall x\,P(x)\equiv\exists x\,\neg P(x)\qquad \neg\exists x\,P(x)\equiv\forall x\,\neg P(x)$$
> **"They also can be applied in Nested Quantifiers."**（对嵌套量词同样适用，逐层往里推。）

> [!example] 课件例题：求 $\forall x\,\exists y\,(xy=1)$ 的否定
> $$\begin{aligned}
> \neg\forall x\,\exists y\,(xy=1)&=\neg\forall x\,\bigl(\exists y\,(xy=1)\bigr)\\
> &=\exists x\,\neg\bigl(\exists y\,(xy=1)\bigr)\\
> &=\exists x\,\bigl(\forall y\,\neg(xy=1)\bigr)\\
> &=\exists x\,\forall y\,(xy\ne1)
> \end{aligned}$$
> **课件的中文注解**：
> - 原式：**"对每个 $x$，都有某个 $y$ 能让 $xy=1$ 成立"**
> - 否定：**"存在某个 $x$，对所有 $y$ 都无法让 $xy=1$ 成立"**（这个 $x$ 就是 $0$）
>
> **要点：否定就是从左往右，把每个量词翻个面，最后把否定推到谓词上。** 量词的**顺序不变**，只是类型逐个翻转。
...
> [!info] 来源
> 本节内容来自课堂录音转写稿 `transcript/1-8节课.txt`（第 6、7 节课）。
...
> [!warning] 这两条是本节最值钱的信息
> **① 翻译题必考。** 讲完「恰好一个」的四种情形后，老师直接说：
> **"这种自然语言的翻译工作，肯定会是你们期中考试的题目。"**
>
> **② 有限论域展开题当过期末考题。** 讲到「把 $\forall x\exists y\,P(x,y)$ 在 $\{1,2,3\}$ 上展开」时，老师说：
> **"这道题好像有一年被我拿来做期末考试的题了，今年我把它拿来做例题。"**
> 而且他明确了**答案要写到什么程度**——
> **"如果题目没给出 $P$ 的具体含义，那你现在看到的这个展开式（9 个 $P(i,j)$ 组成的合取套析取）就是我期末想看到的最终答案。"**

> [!example] 那道题的完整答案（按老师要求的格式）
> 论域为不超过 3 的正整数：
> $$\forall x\,\exists y\,P(x,y)\equiv\bigl[P(1,1)\vee P(1,2)\vee P(1,3)\bigr]\wedge\bigl[P(2,1)\vee P(2,2)\vee P(2,3)\bigr]\wedge\bigl[P(3,1)\vee P(3,2)\vee P(3,3)\bigr]$$
> **步骤要写两层**：先把外层 $\forall x$ 展成三个合取项（老师板书时记作 $Q(1)\wedge Q(2)\wedge Q(3)$），再把每个 $Q(i)=\exists y\,P(i,y)$ 展成三项析取。

---

## T2. ⭐ 两条书写规范（课堂上特别提醒）

> [!warning] ① $\equiv$ 的优先级最低——往年期末真的有人看错
> 讲含常量 $A$ 的等价式 $\forall x\,P(x)\to A\equiv\exists x\,(P(x)\to A)$ 时，老师停下来提醒：
> **"别忘了，$\equiv$ 这个等价符号的优先级是最低的，不要看差了。"**
> **"过去几年有些同学在期末比较紧张，看差了，就问'怎么先要证明 $A$ 跟什么等价啊'——就发生这个问题。"**
>
> **读法：$\forall x\,P(x)\to A\ \equiv\ \exists x\,(P(x)\to A)$ 应理解为 $\bigl(\forall x P(x)\to A\bigr)\equiv\bigl(\exists x(P(x)\to A)\bigr)$。**

> [!warning] ② $\equiv$ 与 $\Leftrightarrow$ 可以混用，但**一份答卷里要统一**
> **"三条杠和双箭头都是同样等效的，没有问题。但是你在做推导的时候，要统一——
> 不能一下突然跳个 double arrow，然后下一行又变成三条杠。"**

---

## T3. 「恰好一个」：课堂上讲的、课件没写的几点

> [!important] Case 1 的两个版本为什么等价
> 课件并排给出两个版本，课堂上老师点明了原因：
> $$\underbrace{z\ne x\to\neg L(\text{Mary},z)}_{\neg p\,\to\,\neg q}\qquad\text{与}\qquad\underbrace{L(\text{Mary},z)\to z=x}_{q\,\to\,p}$$
> **"它们是逆否命题（contrapositive）的关系，所以 Version 1 和 Version 2 是完全一样的。"**
> ——这正是 [[C1.3 命题等价 Propositional Equivalences]] 的 $p\to q\equiv\neg q\to\neg p$。

> [!example] ⭐ Case 4 的证明里，$\forall y$ 为什么能消掉、$\forall w$ 为什么不能
> 这是课堂上花时间最长的一处推导，也是**自由/约束变元概念第一次真正派上用场**。
>
> **能消的那个**：把 $\forall y$ 挪进第二个合取项 $\forall z\,(\forall w\,L(z,w)\to z=x)$ 时，
> **里面根本没有出现 $y$** ⇒ $y$ 对这部分是**自由变元** ⇒ 加不加 $\forall y$ 都不影响推导 ⇒ **可以消掉**。
> 老师的原话：**"因为对于所有的 $y$，加不加我都不影响这个部分的推导。"**
>
> **不能消的那个**：换成 $\forall w$ 时，**$w$ 确确实实出现在 $L(z,w)$ 里** ⇒ $w$ **参与了推导** ⇒ **不能消**。
> 老师当场点名提问，学生答"没有参与"，老师确认：**"很好——所以上面那个和下面那个其实不一样。"**
>
> **判据一句话：量词能不能移动/删除，只看它管的那个变量在里面出没出现。**

> [!important] ⭐ $\forall x\,(P(x)\to A)$ vs $\forall x\,P(x)\to A$ 的中文直觉（课堂原创）
> 设 $P(x)$: "$x$ 努力工作"，$A$: "中国会强大"。
> | 式子 | 中文读法 | 强弱 |
> |---|---|:-:|
> | $\forall x\,(P(x)\to A)$ | **"对每个人来说，只要他努力，中国就会强大"**——允许有人不努力 | 弱 |
> | $\forall x\,P(x)\to A$ | **"如果所有人都努力，中国才会强大"**——只要有一个人不努力就不行 | 强 |
>
> 所以 $\forall x\,(P(x)\to A)\equiv\boxed{\exists x\,P(x)\to A}$——**前件里的 $\forall$ 要翻成 $\exists$**。
> 老师反复用这个例子，**因为这是本节最容易记反的一条**。

---

## T4. 🔧 课件勘误（学生指出、老师确认）

> [!warning] Case 4 早期版本把 $L$ 的两个参数写反了
> 第 7 节课下半场一开始，老师说：
> **"有同学告诉我这里有 typo——之前的版本我写成了 $L(y,x)$，其实不对，应该是 $L(x,y)$。同样的 typo 还有一处。"**
>
> **记法**：本节约定 $L(x,y)$ = "**$x$ loves $y$**"，**第一个参数是施动者**。
> 写 Case 2（恰好一个人爱 Mary）时是 $L(x,\text{Mary})$，写 Case 1（Mary 恰好爱一个人）时是 $L(\text{Mary},x)$——**顺序写反，题意就反了**。
