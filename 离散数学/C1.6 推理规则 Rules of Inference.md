...
> 课件独有的内容（**推理 vs 等价的对照表**、**⇒ 链式书写法**）已并入 §2、§3 正文，标 📚。
...
> [!example] 📚 课件开场：两个“看起来对但其实错”的论证（Part1 slide 3–4，都打了红叉）
> **反例一**
> > Cai Xukun is a singer.　Cai Xukun knows playing basketball.
> > **Therefore, all singers know playing basketball.** ✗
>
> **错在哪**：从**一个**个体的性质推到**全体**——把 $P(a)\wedge Q(a)$ 当成了 $\forall x(P(x)\to Q(x))$。
> 正确只能得到 $\exists x\,(P(x)\wedge Q(x))$（**存在推广 EG**，见 §6）。
>
> **反例二**
> > Some students work hard to study.　Some students fail in examinations.
> > **So, some work hard students fail in examinations.** ✗
>
> **错在哪**：$\exists x\,P(x)\wedge\exists x\,Q(x)\ \nrightarrow\ \exists x\,(P(x)\wedge Q(x))$——用功的学生和挂科的学生**可以是两拨人**。
>
> 这两个反例分别预告本节两个部分：**反例一**指向量化推理规则（§6），**反例二**指向“哪些推理形式有效”。
...
> [!important] ⭐ 📚 课件把“有效”翻译成一句可操作的话（Part1 slide 6）
...
>
> 课件在旁边配了 $p\to q$ 的四行真值表，**只用红框圈出第二行**（$p=\mathbf T,\ q=\mathbf F$），批注 **“Focus on this case — check if it happens.”**
>
> **所以判定有效性不用列完整真值表**，只问一句：
> $$\textbf{“有没有可能所有前提都真、而结论假？”}$$
> **没有可能** ⇒ 有效；**举得出这样一组赋值** ⇒ 无效，而且这组赋值就是**反例**。
> 这与 [[C1.1 命题逻辑 Propositional Logic]] 里 Wason 选择任务的结论完全一致。

> [!warning] ⭐ 🎤 去年期末直接考过“什么是 tautology”
> 讲到“论证有效 $\iff(p_1\wedge\cdots\wedge p_n)\to q$ 是永真式”时，课堂原话：
> **“我在去年的期末考试里就直接问了什么是 tautology——非常简单的一个概念，但有一小部分人没答上来。”**
> **“如果你连 tautology 都不知道，那你在做这种推理题时就没有任何理论基础了。”**
>
> **务必能默写**：永真式 tautology = 无论命题变元取什么真值都**恒为真**的复合命题。
...
> [!important] ⭐⭐ 📚 课件独有：**推理 vs 等价**对照表（Part2 slide 10）
> 这张表教材没有，但它回答了一个很多人到这里会犯迷糊的问题：**$\to$ 和 $\leftrightarrow$ 到底差在哪。**
>
> | | **Inference 推理（$p\to q$）** | **Equivalence 等价（$p\leftrightarrow q$）** |
> |---|---|---|
> | **含义** | If $p$, then $q$ | $p$ is equal to $q$ |
> | **反向** | $p\to q$ **不**意味着 $q\to p$ | $p\leftrightarrow q$ **就**意味着 $q\leftrightarrow p$ |
> | **可用的规则** | 推理规则和等价规则**都能用** | **只能用等价规则** |
> | **相互关系** | 只蕴涵单向 | 要靠证 $p\to q$ **和** $q\to p$ |
> | **证明中的符号** | $\Rightarrow$ | $\Leftrightarrow$ |
>
> 课件结论（红字）：**“Equivalence ($\leftrightarrow$) is a more restrictive relation than Inference ($\to$).”**
>
> **实战含义**：用 $\Leftrightarrow$ 的每一步都**可逆**，用 $\Rightarrow$ 的步骤**不可逆**。
> **能用 $\Leftrightarrow$ 就别用 $\Rightarrow$**（保留信息）；只需要推出结论时，$\Rightarrow$ 就够了，**不必强行找等价**。
...
> [!warning] ⭐⭐ 🎤 课堂：结论是复合表达式时，必须推出**整个表达式**（一个学生的提问问出来的）
> 那道练习的结论是 $\neg q\to s$。学生问：**“我推出了 $\neg q$ 为真、$s$ 为真，然后把它们组合成 $\neg q\to s$，这样可以吗？”**
> 回答是**不行**：
> **“我最后要的不是 $\neg q$ 为真或 $s$ 为真，我要的是整个 expression 为真。”**
> **“上一道题它的结论只有一个 $t$，那个 $t$ 恰好就是一个完整的表达式，所以刚好对应上了。”**
>
> - 结论是 $t$ 这种**单个命题** ⇒ 推出 $t$ 就完事；
> - 结论是 $\neg q\to s$ 这种**复合表达式** ⇒ **必须把整个表达式推出来**。本题的正确路线：$p\to q$ 取逆否得 $\neg q\to\neg p$，再与 $\neg p\to r$、$r\to s$ 连续用**假言三段论**串成 $\neg q\to s$。

> [!important] ⭐⭐ 🎤 答题规范：这已经是第三次强调同一件事
> | 时间 | 场合 | 原话 |
> |---|---|---|
> | 第 1 节课 | 怎么学好这门课 | “你必须明确告诉我用的是什么 theory……记不住名字就写公式” |
> | 第 4 节课 | §1.3 推导题 | “推导的每一个步骤，你必须写清楚用的是什么定律” |
> | **第 8 节课** | §1.6 推理题 | **“我强调一遍，请大家务必记住——这会影响你是否能拿到完整的分数。”** |
>
> **格式**：`步骤内容　|　规则名 或 规则的表达式`。
> 为什么可以写表达式代替名字：**“因为那些规则本身都相对简单，你把公式写在旁边，我就看得懂你用的是哪条。”**

> [!important] 📚 课件的第二种书写法：**⇒ 链式**（Part2 slide 14、18、24、28）
> 把**全部前提合取起来**写成一行，每一步用 $\Rightarrow$ 往下推，右边注明规则：
> $$\begin{aligned}
...
> | **缺点** | 占地方 | 行很长，容易抄错 |
> | **适合** | 前提多、步骤多 | 步骤少（3–5 步） |
>
> 🎤 课堂评论：**“⇒ 链式这种形式更多学生喜欢，因为它显得更干净，像做计算题一样一步一步推。”**
> **但考试建议写分步表格**——它是教材和参考答案的标准格式，**每一步都有独立得分点**；链式一旦中间抄错，后面全错。
> ⇒ 链式**适合打草稿**：它能让你随时看到哪些前提还没用上，而**没用上的前提往往就是下一步的线索**。
> 无论用哪种写法，**每一步右边的规则名/公式都不能省**。

> [!warning] 🎤 $\neg p\wedge q\ \vdash\ \neg p$ 用的是**化简律**，不是否定后件式
> 课堂上预判了这个常见误解：**“你可能问，化简律的标准形式里没有这个 not 啊？——加上 not 一样可以推导，不要把它当作否定后件式。”**
...
> [!note] 🎤 课堂：resolution 的词义与直觉
> **“resolution 有 dissolve（溶解）的意思——可以理解为某些符号在推导里被消解掉了。”** 所以中文叫**归结 / 消解**。
> $$\frac{p\vee q\quad \neg p\vee r}{\therefore\ q\vee r}$$
> **被消解掉的就是 $p$**（它在两个前提里一正一反）。课堂用的例子：
> **“我去游泳或我打网球” + “我不去游泳或我踢足球” ⇒ “我打网球或我踢足球”**——**“游泳这件事对结论已经没有影响了，被消掉了。”**

> [!warning] 🎤 再次提醒：$\mathbf T$ / $\mathbf F$ 禁止当变量用
> 讲归结的真值分析时又强调了一次：**“$\mathbf T$ 和 $\mathbf F$ 有非常特殊的含义，禁止用来代表变量。”**（第一次是在 §1.3。）
...
> [!warning] ⭐ 📚 课件用红框标出 UG 的要求（Part2 slide 20）
> **“Be noted that $b$ that we select must be an arbitrary, and not a specific.”**
> 用 **Universal Generalization** 时选的那个 $b$ 必须是**任意的**，不能是**特定的**。课件单独框出，说明是**考点**。
> **口诀：EI 给你的 $c$ 是“某个特定的”，不能拿去做 UG；UG 要的 $b$ 必须是“随便哪个都行”的。**
> 所以一个证明里 **EI 必须先于 UI 使用**。

> [!example] 📚 课件在“没读书的学生”那题上加的一句重要提醒（Part2 slide 26）
> 前提：“A student in this class has not read the book” + “Everyone in this class passed the first exam”；
> 结论：“Someone who passed the first exam has not read the book”。
>
> 课件蓝字批注：**“We cannot define the domain as student in this class since the conclusion means anyone.”**
> **所以论域必须取“任何人”**，再用 $C(x)$: “$x$ 在本班” 把限制写成谓词：
> $$\exists x\,(C(x)\wedge\neg RB(x)),\quad \forall x\,(C(x)\to PE(x))\ \vdash\ \exists x\,(PE(x)\wedge\neg RB(x))$$
> **要点：论域的选择由结论决定，不是由前提决定。**
...
> [!important] 🎤 课堂总结：从一段自然语言到结论的五步链条
> 1. **判断哪些句子是 statement / proposition**；
> 2. **给每个命题指派字母** $p,q,r,\ldots$（建议先列一张对照表）；
> 3. **用联结词把它们连成表达式**，前提和结论都写成符号；
> 4. **用推理规则和等价律逐步推导**，每步注明依据；
> 5. **看结论（整个表达式）是否被推出**。
>
> 课堂原话：**“这个就是我们完整的一个做证明题的链条。”**

> [!note] 📚 课件收尾：什么是“形式证明”，以及为什么下一节要换路子（Part2 slide 32）
> Proposition、Operator、Predicates、Quantifier、Truth Table、Rules of Equivalence、Rules of Inference——这一整套合起来叫 **formal proof（形式证明）**：
> **very clear and precise**（非常清晰精确），但 **extremely long and hard to follow**（极其冗长、难以阅读）。
> 正因为“长得没法读”，§1.7 才要转向**非形式证明 (informal proof)**——数学书上写的那种证明。见 [[C1.7 证明导论 Introduction to Proofs]]。
...
> 来源：课件 `Ch1.6` Part 1（2026-09-15，8 张）＋ Part 2（2026-09-16，33 张）＋ 课堂录音转写稿第 7、8 节课。
...
>
> > [!success]- 参考答案
> > **a)** $\dfrac{p}{\therefore p \vee q}$ ⇒ **附加律 Addition**
> > **b)** $\dfrac{p \wedge q}{\therefore p}$ ⇒ **化简律 Simplification**
> > **c)** $\dfrac{p \to q,\ p}{\therefore q}$ ⇒ **肯定前件式 Modus ponens**
> > **d)** $\dfrac{p \to q,\ \neg q}{\therefore \neg p}$ ⇒ **否定后件式 Modus tollens**
> > **e)** $\dfrac{p \to q,\ q \to r}{\therefore p \to r}$ ⇒ **假言三段论 Hypothetical syllogism**
